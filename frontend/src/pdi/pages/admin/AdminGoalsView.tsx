...
import { GoalGovernance } from '@pdi/components/GoalGovernance';
...
export function AdminGoalsView({ simplified = false }: { simplified?: boolean }) {
    ...
    const [tableCampusFilter, setTableCampusFilter] = useState<string>("all");
    ...
    const campusStats = React.useMemo(() => {
        ...
        goals.forEach(goal => {
            ...
            if (isRefFilled) grouped[campus].reflectionFilled += 1;
            if (isSetFilled) grouped[campus].settingFilled += 1;
    ...
    const filteredGoals = goals.filter(g => {
        ...
        const matchesCampus = (selectedCampusFilter === "all" || g.campus === selectedCampusFilter) &&
            (tableCampusFilter === "all" || g.campus === tableCampusFilter);
            
        const isRefFilled = g.selfReflectionForm && !g.selfReflectionForm.includes('"adminOverride":true');
        const isSetFilled = g.goalSettingForm && !g.goalSettingForm.includes('"adminOverride":true');
        const isPending = !isRefFilled || !isSetFilled;
        
        let matchesStatus = true;
        if (tableStatusFilter === "PENDING") matchesStatus = isPending;
        else if (tableStatusFilter === "FILLED") matchesStatus = !isPending;
    ...
    return (
        <div className="space-y-6">
            ...
            {!simplified && <GoalGovernance />}

            {/* Campus Aggregated Metrics */}
            {simplified ? (
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <PageHeader
                        title="Campus Goal Progress"
                        subtitle="School-wise goal setting and self-reflection completion tracking"
                    />
                    <div className="flex flex-col gap-2 min-w-[200px] mb-8">
                        <Label className="text-xs font-bold capitalize text-muted-foreground">Filter by School</Label>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                    <h3 className="text-xl font-bold">Campus Progress Overview</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {campusStats
                    .filter(stat => selectedCampusFilter === "all" || stat.campus === selectedCampusFilter)
                    .map(stat => (
                        <Card
                            key={stat.campus}
                            className="shadow-lg hover:shadow-xl transition-all cursor-pointer bg-white hover:bg-muted/5"
                            onClick={() => setSelectedCampusStats(stat.campus)}
                        >
                            <CardHeader className="border-b bg-muted/10 pb-4">
                                <CardTitle className="text-lg flex justify-between">
                                    <span>{stat.campus}</span>
                                    {!simplified && <span className="text-sm text-muted-foreground font-normal">{stat.totalGoals} Total Goals</span>}
                                </CardTitle>
                            </CardHeader>
                ...
                {campusStats.filter(stat => selectedCampusFilter === "all" || stat.campus === selectedCampusFilter).length === 0 && !isLoading && (
                    <div className="col-span-full p-8 text-center text-muted-foreground border-dashed border rounded-xl">No goals assigned yet in this school.</div>
                )}
            </div>
                        ...
                        <CardContent className="p-0">
                            <Table>
                                <TableHeader className="bg-muted/30">
                                    <TableRow>
                                        <TableHead className="font-bold text-zinc-900">Teacher</TableHead>
                                        <TableHead className="font-bold text-zinc-900">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <select 
                                                    ...
                                                    value={tableCampusFilter}
                                                    onChange={(e) => setTableCampusFilter(e.target.value)}
                                        ...
                                        <TableHead className="font-bold text-zinc-900">Goal Title</TableHead>
                                        <TableHead className="font-bold text-zinc-900">
                                            <div className="flex items-center gap-1.5">
                                                <span>Status</span>
                                                <select 
                                                ...
                                                >
                                                    ...
                                                    <option value="PENDING">Some Pending</option>
                                                    <option value="FILLED">Both Filled</option>
                                                </select>
                                            </div>
                                        </TableHead>
                                        <TableHead className="font-bold text-zinc-900">Goal Setting (HOS)</TableHead>
                                        ...
                                        <TableRow key={goal.id} className="hover:bg-muted/10">
                                            ...
                                            <TableCell>
                                                {goal.goalSettingForm
                                                    ? (goal.goalSettingForm.includes('"adminOverride":true') ? <span className="text-warning font-medium text-xs bg-warning/10 px-2 py-1 rounded-md">Admin Overridden</span> : <span className="text-info font-medium text-xs bg-info/10 px-2 py-1 rounded-md">Filled</span>)
                                                    : <span className="text-muted-foreground text-xs italic">Pending</span>}
                                            </TableCell>
                    ...
                    <ScrollArea className="flex-1 pr-4 -mr-4">
                        <div className="space-y-6 pt-4">
                            {(() => {
                                ...
                                const reflectionCompleted = campusGoals.filter(g => g.selfReflectionForm && !g.selfReflectionForm.includes('"adminOverride":true'));
                                const reflectionPending = campusGoals.filter(g => !g.selfReflectionForm || g.selfReflectionForm.includes('"adminOverride":true'));

                                const settingCompleted = campusGoals.filter(g => g.goalSettingForm && !g.goalSettingForm.includes('"adminOverride":true'));
                                const settingPending = campusGoals.filter(g => !g.goalSettingForm || g.goalSettingForm.includes('"adminOverride":true'));

                                return (
                                    <>
                                        <div className="space-y-4">
                                            <h4 className="font-bold flex items-center gap-2 text-sm capitalize text-muted-foreground border-b pb-2">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div className="bg-rose-50 border border-rose-100 p-3 rounded-lg">
                                                    <p className="text-xs font-bold text-rose-700 capitalize mb-2">⏳ Pending ({reflectionPending.length})</p>
                                                    <div className="space-y-1">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div className="bg-amber-50 border border-amber-100 p-3 rounded-lg">
                                                    <p className="text-xs font-bold text-amber-700 capitalize mb-2">⏳ Pending ({settingPending.length})</p>
                                                    <div className="space-y-1">

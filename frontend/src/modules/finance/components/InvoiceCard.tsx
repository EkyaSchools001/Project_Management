...
const statusConfig: Record<string, { icon: any; color: string; label: string }> = {
  ...
  Sent: { icon: Clock, color: 'bg-red-500/20 text-red-500', label: 'Sent' },
  Paid: { icon: CheckCircle, color: 'bg-red-500/20 text-red-500', label: 'Paid' },
...
export function InvoiceCard({ invoice, onPay, onVoid, onClick }: InvoiceCardProps) {
  ...
  return (
    <div 
      onClick={onClick}
      className="bg-[#1a1d24] rounded-2xl p-6 border border-white/5 hover:border-[#ef4444]/30 transition-all"
    >
        ...
        <div className="flex gap-2">
          {onPay && invoice.status === 'Sent' && (
            <button
              ...
              className="flex-1 px-4 py-2 bg-[#ef4444] text-black rounded-xl font-bold text-sm hover:bg-[#ef4444]/80 transition-colors"
            >
              Mark Paid
            </button>
          )}

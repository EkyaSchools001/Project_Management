...
const MorningMeetingsPage = () => {
  ...
  const [data, setData] = useState({
    ...
    mainTitle: "Morning",
    italicTitle: "Meetings",
    heroDesc: "Nurturing social-emotional wellbeing and building classroom community every day.",
    introText: "Morning Meetings are conducted in the classroom on days when there is no assembly. They address students' social-emotional learning needs: building community, trust, and a positive tone for learning. They are led by the class teacher and last 10 minutes. Each meeting follows a consistent structure that helps students feel seen, grounded, and ready to learn.",
    differentiation: "Substitute teachers follow the same Morning Meeting structure if the class teacher is absent. For Grades 1–4, use the Greetings Poster at the classroom entrance to let students choose their greeting. Early Years classrooms follow the Circle Time format instead.",
    quickTip: "Prepare your Good Things example in advance each morning: when teachers share something genuine and personal, students quickly feel safe to open up too.",
    processSteps: [
      { text: "Step 1 – Greeting: Greet each student at the door with eye contact, their name, and a handshake/high five/fist bump." },
      { text: "Step 2 – Check-in: Ask 1–2 students how they are feeling; check diary notes from parents." },
      { text: "Step 3 – Prayer: Begin with the school prayer in a calm, clear tone." },
      { text: "Step 4 – Good Things: Share a Good Thing yourself; invite 2 students to share (rotate students across the week)." },
      { text: "Step 5 – Grounding/Resourcing Activity: Lead a brief mindfulness or calming activity (breathing, stretching, or reflection)." },
      { text: "Step 6 – Agenda & Announcements: Share the day's plan, any schedule changes, and upcoming events." },
      { text: "Step 7 – Positive Intention: Set a positive intention to anchor the class's focus and mindset for the day." }
    ],
    dos: [
      { text: "Use the greetings poster for Grades 1–4 to give students agency in their greeting" },
      { text: "Model Good Things before inviting students to share" },
      { text: "Pick different students each day for Good Things : track your rotation" },
      { text: "Choose a grounding activity from the shared resource bank" },
      { text: "Keep Morning Meeting to 10 minutes : tight structure matters" }
    ],
    donts: [
      { text: "Don't skip Morning Meeting : it sets the emotional tone for the entire day" },
      { text: "Don't call only enthusiastic sharers : include quieter students gradually" },
      { text: "Don't allow prayer to feel rushed or routine : set a calm pace" },
      { text: "Don't react negatively if a student shares something unexpected" },
      { text: "Don't let Meeting run into lesson time : be disciplined about the 10-minute window" }
    ],
    glossary: [
      { term: "Morning Meeting", mean: "A structured 10-minute classroom practice addressing social-emotional learning at the start of each school day." },
      { term: "Good Things", mean: "A warm-up practice where students share a positive experience, building a sense of community." },
      { term: "Grounding Activity", mean: "A brief calming or mindfulness exercise (e.g., breathing, stretching) to help students settle and focus." },
      { term: "Positive Intention", mean: "A motivating thought shared at the start of the day to guide students' attitude and actions." },
      { term: "SEL", mean: "Social-Emotional Learning : the process of developing self-awareness, empathy, and relationship skills." }
    ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_morning_meetings");
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls
        settingKey="page_culture_morning_meetings"
        initialData={data}
        onSave={setData}
        title="Morning Meetings Content"
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-24">
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">{data.processSubtitle || "The Routine"}</h2>
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">{data.differentiationSubtitle || "Contextual Use"}</h2>
...
export default MorningMeetingsPage;

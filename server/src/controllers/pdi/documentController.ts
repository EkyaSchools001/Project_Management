...
export const getAllDocuments = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const documents = await prisma.pDIDocument.findMany({
...
export const uploadDocument = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const document = await prisma.pDIDocument.create({
            data: {
                title,
                description,
...
export const assignDocument = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        for (const teacherId of teacherIds) {
            const existing = await prisma.pDIDocumentAcknowledgement.findFirst({
                where: { documentId, teacherId }
            });

            if (!existing) {
                const newAck = await prisma.pDIDocumentAcknowledgement.create({
...
export const getTeacherAcknowledgements = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const acknowledgements = await prisma.pDIDocumentAcknowledgement.findMany({
            where: { teacherId },
            include: {
                document: {
                    include: {
...
export const acknowledgeDocument = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const ack = await prisma.pDIDocumentAcknowledgement.findUnique({
            where: { id: ackId }
        });
        ...
        const updatedAck = await prisma.pDIDocumentAcknowledgement.update({
            where: { id: ackId },
            data: {
                status: 'ACKNOWLEDGED',
...
export const deleteDocument = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        await prisma.pDIDocument.delete({
            where: { id }
        });

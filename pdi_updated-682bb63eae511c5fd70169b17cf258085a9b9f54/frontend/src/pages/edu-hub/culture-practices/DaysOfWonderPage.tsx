...
const DaysOfWonderPage = () => {
  ...
  const [data, setData] = useState({
    ...
    mainTitle: "Days of",
    italicTitle: "Wonder",
    heroDesc: "A joyful and intentional welcome to the new school year for every student.",
    introText: "Days of Wonder are the structured first day (or two days) of school designed to help students build connections with their peers, teachers, and environment. Each grade block has a tailored plan : from Early Years to Senior School : that includes icebreakers, goal-setting, the Social Contract, Good Things, and grounding activities. The first day experience sets the emotional tone for the entire year.",
    differentiation: "Grades 9–12 have a longer Day 1 (full day with lunch); Grades 6–8 have a half-day on Day 1 and a full day on Day 2; Grades 3–5 and 1–2 follow adapted schedules. Early Years (Day 1 & 2): Circle Time, colouring, rhymes, outdoor play, and Montessori material time : no formal academic content. SJRs disperse at 10:30 am; JRs/SRs/K1/K2 at 11:30 am.",
    quickTip: "Prepare a 'Know My Students' cheat sheet before Day 1 : names, known interests, any key information from last year's teacher : being able to call every student by name on the first day is a powerful relationship signal.",
    processSteps: [
      { text: "Greet students at the gate and guide them to the assembly area on arrival." },
      { text: "Run the Welcome Assembly: prayer, HoS welcome address, coordinator positive intention." },
      { text: "In Class (Day 1): Good Things, grounding activity, attendance, icebreaker, student calendar walkthrough, assign buddies to new students." },
      { text: "Team-building activity and classroom Responsibilities & Rewards orientation." },
      { text: "Gratitude reflection and FIND Journal introduction." },
      { text: "Goal-setting session: students set SMART goals for the year." },
      { text: "Day 2 (Grades 1–8): Build the Social Contract, creative writing activity, VA/PA/PE/Library session." },
      { text: "End each day with a 'Launch' : a thought-provoking close that sends students home feeling inspired." }
    ],
    dos: [
      { text: "Know your students' names before they arrive on Day 1" },
      { text: "Assign buddies to every new student before the first bell" },
      { text: "Model Good Things with genuine personal sharing" },
      { text: "End each day with the Launch to send students home positively" },
      { text: "Send a Schoology message to students that evening : it signals you care" }
    ],
    donts: [
      { text: "Don't start academic content on Day 1 : relationship-building comes first" },
      { text: "Don't rush through icebreakers : connection takes time" },
      { text: "Don't skip goal-setting : it gives students a sense of direction and ownership" },
      { text: "Don't leave new students without a buddy : isolation on Day 1 has lasting effects" },
      { text: "Don't forget to send the diary note to parents for the next day's Show and Tell or preparation" }
    ],
    glossary: [
      { term: "Days of Wonder", mean: "The structured first day(s) of school focused on community-building, orientation, and connection rather than academic instruction." },
      { term: "SMART Goals", mean: "Goals that are Specific, Measurable, Achievable, Relevant, and Time-bound." },
      { term: "FIND Journal", mean: "A student reflection journal used for mindfulness, gratitude, and self-awareness practices throughout the year." },
      { term: "Buddy", mean: "An existing student assigned to support a new student in navigating the school environment." },
      { term: "Launch", mean: "An inspiring thought-provoking close to each day : a quote, question, or reflection that sends students home engaged." }
    ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_days_of_wonder");
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls 
        settingKey="page_culture_days_of_wonder"
        initialData={data}
        onSave={setData}
        title="Days of Wonder Content"
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-24">
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">{data.processSubtitle || "The First Days"}</h2>
        ...
        <section className="animate-in fade-in zoom-in duration-700">
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-[2.5rem] p-10 md:p-16 border border-amber-200/50 flex flex-col md:flex-row items-center gap-10 shadow-sm relative overflow-hidden">
            ...
            <div className="w-20 h-20 rounded-3xl bg-white shadow-xl flex items-center justify-center flex-none">
              <Rocket className="w-10 h-10 text-amber-500" weight="fill" />
            </div>
...
export default DaysOfWonderPage;

...
const PermissionBadge = ({ permission }) => {
    const config = {
        [PERMISSIONS.ALL]: { label: 'Full Access', color: 'bg-neutral-800 text-white border-transparent' },
        [PERMISSIONS.DEPARTMENTS]: { label: 'Departments', color: 'bg-indigo-50 text-indigo-700 border-indigo-100' },
        [PERMISSIONS.SCHOOLS]: { label: 'Schools', color: 'bg-blue-50 text-blue-700 border-blue-100' },
        [PERMISSIONS.ANALYTICS]: { label: 'Analytics', color: 'bg-emerald-50 text-emerald-700 border-emerald-100' },
        ...
        [PERMISSIONS.GROWTH_TEACHER]: { label: 'GH Teacher', color: 'bg-emerald-50 text-emerald-700 border-emerald-100' },
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 sm:gap-10 py-6 sm:py-10 border-b border-slate-200">
                <div className="space-y-2 w-full sm:w-auto">
                    <div className="flex items-center gap-3">
                        <div className="w-2.5 h-2.5 rounded-full bg-blue-600 animate-pulse shadow-[0_0_15px_rgba(37,99,235,0.3)]"></div>
                ...
                {view === 'users' && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} key="users" className="space-y-8 sm:space-y-12">
                        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                            {[
                                { label: 'Verified Nodes', val: users.length, icon: Users, color: 'text-blue-600 bg-blue-50' },
                                { label: 'Active Clusters', val: '12', icon: Globe, color: 'text-emerald-600 bg-emerald-50' },
                        ...
                        <section className="bg-zinc-900 text-white rounded-[2rem] sm:rounded-[4rem] p-8 sm:p-16 relative overflow-hidden shadow-2xl border border-zinc-800">
                            ...
                            <div className="relative z-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 sm:gap-10">
                                ...
                                <button
                                    ...
                                    className={`w-full sm:w-auto h-16 sm:h-20 px-8 sm:px-16 text-[10px] sm:text-xs font-black rounded-2xl sm:rounded-3xl transition-all shadow-2xl active:scale-95 uppercase tracking-[0.2em] flex items-center justify-center gap-4 ${isEditingPermissions ? 'bg-emerald-500 text-white shadow-emerald-500/20' : 'bg-white text-slate-900 group-hover:bg-blue-50'}`}
                                >
                                    ...
                                    <Card key={key} className={`space-y-6 sm:space-y-10 p-6 sm:p-12 border border-slate-100 shadow-sm bg-white rounded-[2rem] sm:rounded-[3rem] transition-all duration-700 ${isEditingPermissions ? 'ring-4 ring-blue-500/20 scale-[1.02] z-10 border-blue-500' : ''}`}>
                                        <div className="flex items-center justify-between pb-6 sm:pb-10 border-b border-slate-50">
                                            <div className="flex items-center gap-4 sm:gap-8 min-w-0">
                                                <div className="w-12 h-12 sm:w-20 sm:h-20 bg-blue-50 text-blue-600 rounded-2xl sm:rounded-[2rem] flex items-center justify-center font-black text-xl sm:text-3xl border border-blue-100 shrink-0">
                                                    {roleName.charAt(0)}
                                                </div>
                                                <div className="min-w-0">
                            ...
                            <div className="relative z-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 sm:gap-12">
                                <div className="space-y-10 max-w-4xl text-left">
                                    <div className="flex items-center gap-4">
                                        <div className="w-3 h-3 rounded-full bg-blue-600 animate-pulse shadow-[0_0_15px_rgba(37,99,235,0.4)]"></div>
                            ...
                            {users.map((user) => (
                                <Card key={user.id} className="p-8 sm:p-12 space-y-8 sm:space-y-10 group hover:border-blue-500 transition-all duration-700 border border-slate-200 shadow-sm relative overflow-hidden bg-white rounded-[2rem] sm:rounded-[3.5rem] border-t-4 sm:border-t-8 border-t-transparent hover:border-t-blue-500">
                                    ...
                                    <div className="flex items-center gap-6 sm:gap-8 border-b border-slate-100 pb-8 sm:pb-10">
                                        <div className="w-16 h-16 sm:w-20 sm:h-20 bg-blue-50 text-blue-600 rounded-2xl sm:rounded-[2rem] flex items-center justify-center font-black text-2xl sm:text-3xl border border-blue-100 shrink-0">
                                            {user.name.charAt(0)}
                                        </div>
                                        <div className="min-w-0">
                                    ...
                                    <div className="space-y-6 sm:space-y-8">
                                        ...
                                        <div className="grid grid-cols-1 gap-3 sm:gap-4">
                                            {[
                                                ...
                                                { id: PERMISSIONS.GROWTH_TEACHER, label: 'Practitioner', icon: GraduationCap, color: 'text-emerald-600 bg-emerald-50' }
                                            ].map(role => {
                                    ...
                                    <div className="pt-8 sm:pt-10 border-t border-slate-50 flex items-center justify-between gap-4 group/toggle">
                                        ...
                                        <button
                                            ...
                                            className={`w-16 sm:w-20 h-8 sm:h-10 rounded-full transition-all relative shrink-0 ${(user.overrides || currentPermissions[user.role] || []).includes(PERMISSIONS.GROWTH_HUB) ? 'bg-blue-600 shadow-xl shadow-blue-600/20' : 'bg-slate-200'}`}
                                        >
                ...
                {view === 'audit' && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} key="audit" className="space-y-8 sm:space-y-12">
                        ...
                        <div className="space-y-4 sm:space-y-6">
                            {[
                                { user: 'superadmin@ekya.edu', action: 'Update Matrix', target: 'SuperAdmin Role', time: '12:45:01 PM', status: 'Success', icon: ShieldCheck, color: 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20' },
                                ...
                                { user: 'hr.manager@ekya.edu', action: 'Batch Provision', target: '12 Identities', time: '08:02:44 AM', status: 'Success', icon: Users, color: 'text-brand-500 bg-brand-500/10 border-brand-500/20' },
                                    ...
                                    <div className="flex items-center gap-6 sm:gap-10 w-full md:w-auto">
                                        ...
                                        <div className="space-y-1 sm:space-y-2 min-w-0">
                                            <div className="flex flex-wrap items-center gap-3">
                                                ...
                                                <span className={`text-[8px] sm:text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${log.status === 'Success' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                                                    {log.status}
                                                </span>
                                            </div>
                        ...
                        >
                            ...
                            <div className="overflow-y-auto custom-scrollbar pr-2 flex-1">
                                <div className="mb-12 sm:mb-20 text-center relative pt-4 sm:pt-0">
                                    <div className="w-24 h-24 sm:w-40 sm:h-40 bg-blue-50 text-blue-600 border border-blue-100 rounded-[2rem] sm:rounded-[3rem] flex items-center justify-center text-4xl sm:text-7xl font-black mx-auto mb-6 sm:mb-10 shadow-lg relative overflow-hidden">
                                    ...
                                    <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4">
                                        <div className="flex items-center gap-2 sm:gap-3 bg-blue-50 px-4 sm:px-8 py-2 sm:py-3 rounded-full border border-blue-100">

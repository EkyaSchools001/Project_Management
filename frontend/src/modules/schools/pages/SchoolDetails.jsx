    ...
    return (
        ...
        >
            ...
            <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-12 relative overflow-hidden p-12 sm:p-16 lg:p-24 bg-background rounded-[5rem] text-foreground shadow-2xl group border border-white/5">
                <div className="absolute inset-0 bg-gradient-to-br from-rose-600/30 via-rose-600/10 to-transparent pointer-events-none" />
                ...
                <div className="space-y-10 relative z-10 flex-1 w-full sm:w-auto">
                    ...
                    <div className="space-y-6">
                        <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-6">
                            ...
                            <div className="px-8 py-3 bg-rose-600 text-foreground rounded-full text-[10px] font-black uppercase tracking-[0.4em] shadow-2xl shadow-rose-600/40 border border-white/10">
                                ACTIVE_NODE
                            </div>
                        </motion.div>
                        <motion.p variants={itemVariants} className="text-foreground/40 font-black text-xs sm:text-lg uppercase tracking-[0.2em] flex items-center gap-4">
                            ...
                            Protocol Vector: <span className="text-rose-400 animate-gradient-shift">EKYA_{school.id.toUpperCase()}</span>
                        </motion.p>
                    </div>
                </div>
                ...
                <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center gap-10 bg-white/5 backdrop-blur-3xl border border-white/10 p-10 lg:p-12 rounded-[4rem] shadow-2xl shadow-black/20 w-full xl:w-auto relative z-10 group-hover:bg-white/10 transition-all duration-700">
                    <div className="flex flex-col gap-4 border-b sm:border-b-0 sm:border-r border-white/10 pb-6 sm:pb-0 sm:pr-12 w-full sm:w-auto">
                        <div className="flex items-center gap-4">
                            <div className="w-4 h-4 bg-red-400 rounded-full animate-pulse-glow shadow-[0_0_20px_rgba(52,211,153,0.6)]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
                {[
                    { label: 'Control Center', icon: LayoutDashboard, desc: 'Operational Matrix', color: 'from-red-500 to-rose-600', text: 'text-red-500' },
                    { label: 'Personnel', icon: UserCheck, desc: 'Identity Clusters', color: 'from-red-500 to-red-600', text: 'text-red-500' },
                    { label: 'Identity Vault', icon: GraduationCap, desc: 'Credential Streams', color: 'from-rose-500 to-pink-600', text: 'text-rose-500' },
                    { label: 'Protocol Base', icon: BookOpen, desc: 'Strategic Nodes', color: 'from-red-400 to-rose-600', text: 'text-red-500' }
                ].map((item, i) => (
                    <motion.div key={i} variants={itemVariants}>
                        <Card className="p-12 group cursor-pointer hover:shadow-[0_45px_100px_rgba(0,0,0,0.1)] hover:border-rose-500 transition-all duration-700 bg-white border border-slate-100 rounded-[4rem] relative overflow-hidden flex flex-col items-center text-center space-y-10 group/card">
                            ...
                            <div className={`w-28 h-28 rounded-[2.5rem] bg-slate-50 flex items-center justify-center transition-all group-hover:scale-115 mb-4 shadow-inner group-hover:bg-white group-hover:shadow-2xl group-hover:shadow-rose-500/10 ${item.text}`}>
                                <item.icon size={44} className="group-hover:rotate-12 transition-transform duration-700" />
                            </div>
                            
                            <div className="space-y-4 w-full">
                                <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter group-hover:text-rose-600 transition-colors leading-none">{item.label}</h3>
                            ...
                            <div className="w-full pt-10 border-t border-slate-50 mt-auto flex items-center justify-between">
                                <span className="text-[10px] font-black text-rose-600 uppercase tracking-[0.3em] opacity-0 group-hover/card:opacity-100 transition-all translate-x-6 group-hover/card:translate-x-0">Launch Module</span>
                                <ChevronRight className="text-slate-200 group-hover:text-rose-600 group-hover:translate-x-3 transition-all" size={28} />
                            </div>
                        </Card>
                    </motion.div>
                ))}
            ...
            <motion.div variants={itemVariants}>
                <Card className="bg-slate-950 text-foreground p-16 lg:p-24 flex flex-col xl:flex-row items-center justify-between gap-20 relative overflow-hidden group border-none shadow-[0_60px_150px_rgba(0,0,0,0.3)] rounded-[5rem]">
                    ...
                    <div className="absolute top-0 right-0 p-12 opacity-10 scale-150 rotate-12 transition-transform duration-1000 group-hover:rotate-0 pointer-events-none text-rose-500">
                        <Shield size={600} />
                    </div>

                    <div className="relative z-10 max-w-4xl space-y-12">
                        <div className="inline-flex items-center gap-6 px-10 py-4 bg-rose-600/10 rounded-full border border-rose-400/20 backdrop-blur-xl group-hover:bg-rose-600/20 transition-all">
                            <Cpu size={28} className="text-rose-400 animate-pulse" />
                            <span className="text-[12px] font-black tracking-[0.4em] uppercase text-rose-300">Enterprise Integration Layer v2.0</span>
                        </div>
                        <div className="space-y-8">
                            <h3 className="text-6xl sm:text-8xl font-black uppercase tracking-tighter leading-none">
                                Neural <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-rose-400">Sync Matrix</span>
                            </h3>
                        ...
                        <div className="flex flex-col sm:flex-row gap-8 pt-8">
                            <button className="h-22 px-14 bg-rose-600 text-foreground font-black rounded-[2rem] hover:bg-rose-500 shadow-2xl shadow-rose-600/40 active:scale-95 transition-all uppercase tracking-[0.4em] text-[11px] flex items-center gap-6 group/btn">
                                Initialize Protocol
                    ...
                    <div className="relative z-10 hidden xl:block shrink-0">
                        <div className="w-[450px] h-[450px] border-[30px] border-white/5 rounded-[4rem] flex items-center justify-center relative shadow-inner overflow-hidden">
                            ...
                            <div className="w-full h-full border-4 border-dashed border-rose-500/20 rounded-full animate-spin-slow absolute scale-150 rotate-45" />
                            <div className="w-full h-full border-2 border-rose-500/20 rounded-[3rem] animate-spin-slow absolute -rotate-12" />
                            <div className="relative z-20 flex flex-col items-center gap-6">
                                <Sparkles size={140} className="text-rose-400 group-hover:scale-110 transition-transform duration-1000" />
                                <div className="text-[10px] font-black tracking-[1em] text-rose-400/40 uppercase">AWAITING SYNC</div>
                            </div>
                        </div>
                    </div>
                </Card>
            ...
            <div className="py-20 flex flex-col items-center gap-10">
                <div className="w-40 h-2 bg-slate-100 rounded-full overflow-hidden p-0.5">
                    <motion.div 
                        ...
                        className="h-full bg-gradient-to-r from-rose-600 to-rose-600 rounded-full" 
                    />
                </div>

...
import { assessmentService, Assessment } from "@pdi/services/assessmentService";
...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@pdi/components/ui/dropdown-menu";
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export const AssessmentManagementDashboard: React.FC<AssessmentManagementDashboardProps> = ({ hideHeader = false }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <Tabs defaultValue={isManagement ? "analytics" : "templates"} className="w-full">
                <TabsList className="grid grid-cols-3 w-full max-w-md bg-zinc-100 rounded-xl p-1">
                    {!isManagement && <TabsTrigger value="templates" className="rounded-lg gap-2"><FileText className="w-4 h-4" /> Templates</TabsTrigger>}
                    {!isManagement && <TabsTrigger value="assignments" className="rounded-lg gap-2"><Users className="w-4 h-4" /> Assignments</TabsTrigger>}
                    <TabsTrigger value="analytics" className="rounded-lg gap-2 col-span-1"><BarChart3 className="w-4 h-4" /> Analytics</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="templates" className="mt-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredAssessments.map(assessment => (
                            <Card key={assessment.id} className="shadow-lg hover:shadow-xl transition-all">
                ...
                <TabsContent value="analytics" className="mt-6">
                    {(user?.role === 'ADMIN' || user?.role === 'SUPERADMIN' || user?.role === 'SCHOOL_LEADER' || user?.role === 'LEADER' || user?.role === 'MANAGEMENT') && analyticsData?.users ? (

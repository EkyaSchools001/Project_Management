    ...
    try {
        ...
        for (const attendee of updatedMeeting.attendees) {
            await createNotification({
                ...
                message: `The meeting "${updatedMeeting.title}" on ${updatedMeeting.meetingDate} has been updated.`,
                type: 'INFO',
...
export const deleteMeeting = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // Fetch attendees before deletion to notify them
        ...
        if (meetingWithAttendees) {
            for (const attendee of meetingWithAttendees.attendees) {
                await createNotification({
                    ...
                    message: `The meeting "${meetingWithAttendees.title}" scheduled for ${meetingWithAttendees.meetingDate} has been cancelled.`,
                    type: 'WARNING',
                    link: '/meetings'
                });
            }
...
export const completeMeeting = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // Notify attendees that the meeting is completed (MoM is ready)
        for (const attendee of updatedMeeting.attendees) {
            await createNotification({
                userId: attendee.userId,
                title: 'Meeting Minutes Ready',
                message: `The minutes for "${updatedMeeting.title}" are now available for review.`,
                type: 'SUCCESS',
                link: `/meetings/${updatedMeeting.id}`
            });
        }

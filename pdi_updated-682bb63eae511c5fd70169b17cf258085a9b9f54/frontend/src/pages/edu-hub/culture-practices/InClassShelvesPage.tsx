...
const InClassShelvesPage = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const [isEditorOpen, setIsEditorOpen] = useState(false);
  
  const [data, setData] = useState({
    heroImage: "https://images.unsplash.com/photo-1590402444816-05484f507ffe?q=80&w=2070&auto=format&fit=crop",
    mainTitle: "In-Class",
    italicTitle: "Shelves",
    heroDesc: "Keeping the classroom organised, accessible, and functional throughout the school year.",
    introText: "Every classroom has open shelves and cupboards for storing books, stationery, and student belongings. A well-organised shelf system supports smooth daily routines, minimises disruptions, and builds student responsibility. Setting up shelves correctly in the first week of school ensures a functional learning environment from Day 1.",
    differentiation: "Class Representatives (2 students) are responsible for daily book distribution, collection, and shelf organisation. In Early Years, the shelf organisation follows the Montessori curriculum areas : materials return to their exact designated spots. Teachers should explicitly model shelf use with students in the first week.",
    quickTip: "Create a simple 'Shelf Map' poster for the classroom : a visual guide showing which shelf holds which subject. Students reference it independently, reducing the constant 'Where does this go?' questions.",
    processSteps: [
      { text: "Set up shelves during the first week of school reopening." },
      { text: "Label each shelf section with Learning Area stickers (English, Math, Science, etc.)." },
      { text: "Designate the bottom two rows for students' lunch bags : label them 'Lunch Bags'." },
      { text: "Books and stationery must be labelled with the student's name, class, and section." },
      { text: "Stack books horizontally (one above the other) or vertically (side-by-side) : no piling." },
      { text: "Designate one shelf area as the Class Library : label it clearly." },
      { text: "Use the locked cupboard for items that need security: chart paper, board pins, tape, markers, chalk, scissors." },
      { text: "Do NOT store items on top of the shelf; the top must not be used as a display area." }
    ],
    dos: [
      { text: "Label shelves and books before students arrive on Day 1" },
      { text: "Assign Class Representatives to manage shelf routines from the first week" },
      { text: "Use the locked cupboard for art and stationery supplies" },
      { text: "Check shelf organisation 3 times daily: short break, before lunch, before last period" },
      { text: "Keep the Class Library shelf clearly separated from other subject books" }
    ],
    donts: [
      { text: "Don't store items on top of shelves : this is explicitly not permitted" },
      { text: "Don't leave unlabelled books on shelves : they become lost and disorganised quickly" },
      { text: "Don't use the shelf top as a display area" },
      { text: "Don't allow students to take books without the Class Representative's knowledge" },
      { text: "Don't let lunch bags migrate to subject shelves : keep rows clearly separated" }
    ],
    glossary: [
      { term: "Class Representatives", mean: "Students assigned the duty of managing the classroom, including distributing, collecting, and organising books on shelves." },
      { term: "Learning Area Sticker", mean: "A printed label identifying the subject (English, Math, Science, etc.) for each shelf section." },
      { term: "Class Library", mean: "A designated shelf section holding student-brought books for peer reading and borrowing." },
      { term: "Shelf Organisation", mean: "The structured arrangement of books and materials to make daily classroom routines smooth and efficient." }
    ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_in_class_shelves");
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls 
        settingKey="page_culture_in_class_shelves"
        initialData={data}
        onSave={setData}
        title="In-Class Shelves Content"
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-24">
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">{data.processSubtitle || "Organization Steps"}</h2>
        ...
        <section className="animate-in fade-in zoom-in duration-700">
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-[2.5rem] p-10 md:p-16 border border-amber-200/50 flex flex-col md:flex-row items-center gap-10 shadow-sm relative overflow-hidden">
            ...
            <div className="w-20 h-20 rounded-3xl bg-white shadow-xl flex items-center justify-center flex-none">
              <Archive className="w-10 h-10 text-amber-500" weight="fill" />
            </div>
...
export default InClassShelvesPage;

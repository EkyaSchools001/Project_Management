...
export default function WorkloadChart({ teamMembers = [], tasks = [] }) {
    ...
    const getLoadCategoryBadge = (category) => {
        switch (category) {
            ...
            case 'underutilized':
                return { class: 'bg-red-400/20 text-red-400', label: 'Underutilized' };
            default:
                return { class: 'bg-[#ef4444]/20 text-[#ef4444]', label: 'Optimal' };
        }
    };
    ...
    return (
        <div className="bg-[#161B22] rounded-2xl p-6 border border-white/5">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                <div className="p-4 bg-[#ef4444]/10 rounded-xl border border-[#ef4444]/20 text-center">
                    <p className="text-2xl font-bold text-[#ef4444]">{workloadData?.summary?.optimal || 0}</p>
                    <p className="text-xs text-foreground/50">Optimal</p>
                </div>
                <div className="p-4 bg-red-400/10 rounded-xl border border-red-400/20 text-center">
                    <p className="text-2xl font-bold text-red-400">{workloadData?.summary?.underutilized || 0}</p>
            ...
            {workloadData?.suggestions?.length > 0 && (
                <div className="border-t border-white/10 pt-4">
                    ...
                    {workloadData.suggestions.map((suggestion, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 bg-[#ef4444]/5 rounded-xl border border-[#ef4444]/20">
                            <div className="flex items-center gap-2">

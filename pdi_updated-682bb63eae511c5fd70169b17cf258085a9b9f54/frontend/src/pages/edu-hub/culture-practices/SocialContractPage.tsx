...
const SocialContractPage = () => {
  ...
  const [data, setData] = useState({
    ...
    mainTitle: "Social",
    italicTitle: "Contract",
    heroDesc: "Building classroom ownership, trust, and self-management through shared agreements.",
    introText: "The Social Contract is a collaboratively built agreement between students and their teacher on classroom behaviour norms and consequences. Unlike rules imposed from the top, the Social Contract emerges from student voice: making it far more powerful and owned. It must be built in the first week of school and revisited whenever behaviour concerns arise.",
    differentiation: "Early Years and Grades 1–2 may use a simpler version with pictures and key words rather than full sentences. For senior students (Grades 9–12), include technology use, digital citizenship norms, and conflict resolution explicitly. Contracts can include peer behaviour and teacher behaviour expectations.",
    quickTip: "At the end of each week, do a quick 3-2-1 Social Contract check-in: 3 = followed everything, 2 = some distractions, 1 = significant issues. This 2-minute ritual keeps the contract alive and relevant.",
    processSteps: [
      { text: "Week 1: Divide students into groups of 4–5, assigning roles (scribe, spokesperson, timekeeper, on-task person)." },
      { text: "Give each group four questions: How do you want to be treated by the teacher? By each other? How should the teacher be treated? How do you want to handle conflict?" },
      { text: "Groups discuss and record their responses." },
      { text: "Reconvene as a class; each group shares answers: teacher writes responses on the board." },
      { text: "Consolidate similar responses; guide students to include any missing behavioural objectives." },
      { text: "Finalise the contract (10–25 words/terms); create a poster or anchor chart." },
      { text: "All students sign the contract; teacher signs too; all Learning Area teachers must also sign." },
      { text: "Display the contract prominently in the classroom. Reinforce it throughout the year." }
    ],
    dos: [
      { text: "Build the Social Contract in the first week of school" },
      { text: "Ensure all Learning Area teachers sign the contract" },
      { text: "Display the contract where all students can see it daily" },
      { text: "Refer back to the contract when addressing student behaviour" },
      { text: "Reinforce it positively : celebrate students who uphold it" }
    ],
    donts: [
      { text: "Don't impose norms : the contract must come from students' own words" },
      { text: "Don't build the contract and then ignore it" },
      { text: "Don't leave students out of the signing process" },
      { text: "Don't use the contract punitively as a 'gotcha' : it is a shared guide" },
      { text: "Don't forget to involve all subject teachers : they must sign and honour it too" }
    ],
    glossary: [
      { term: "Social Contract", mean: "A jointly created, student-signed agreement on how everyone in the classroom will treat each other." },
      { term: "Anchor Chart", mean: "A large, visible classroom display that captures key learning or agreements for students to reference." },
      { term: "Behavioural Objective", mean: "A specific, observable behaviour expectation included in the Social Contract." },
      { term: "Norms", mean: "Shared expectations for how people interact within a community or learning space." },
      { term: "Student Voice", mean: "The active participation of students in decisions that affect their learning environment." }
    ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_social_contract");
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls 
        settingKey="page_culture_social_contract"
        initialData={data}
        onSave={setData}
        title="Social Contract Content"
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-24">
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">{data.processSubtitle || "Building the Contract"}</h2>
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">{data.differentiationSubtitle || "Contextual Adaptation"}</h2>
        ...
        <section className="animate-in fade-in zoom-in duration-700">
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-[2.5rem] p-10 md:p-16 border border-amber-200/50 flex flex-col md:flex-row items-center gap-10 shadow-sm relative overflow-hidden">
            ...
            <div className="w-20 h-20 rounded-3xl bg-white shadow-xl flex items-center justify-center flex-none">
              <UsersFour className="w-10 h-10 text-amber-500" weight="fill" />
            </div>

    ...
    return (
        ...
        >
            ...
            <section className="relative overflow-hidden p-12 sm:p-16 lg:p-24 bg-background rounded-[5rem] text-foreground shadow-2xl group border border-white/5">
                ...
                <div className="absolute -right-20 -top-20 w-[800px] h-[800px] bg-rose-500 rounded-full blur-[200px] opacity-10 pointer-events-none group-hover:scale-125 transition-transform duration-1000" />
                ...
                <div className="flex flex-col xl:flex-row justify-between items-start gap-16 relative z-10">
                    <div className="flex-1 space-y-12">
                        <motion.button
                            ...
                            className="group flex items-center text-[11px] font-black text-foreground/40 hover:text-rose-400 transition-all uppercase tracking-[0.4em] active:scale-95"
                        >
                        ...
                        <div className="space-y-8">
                            <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-8">
                                ...
                                <div className="px-8 py-3 bg-white/5 backdrop-blur-xl border border-white/10 text-rose-400 rounded-full text-[10px] sm:text-[11px] font-black uppercase tracking-[0.4em]">
                        ...
                        <div className="flex flex-wrap gap-8 pt-10">
                            <StatBadge icon={User} label="Command Lead" value={project.manager.name} color="text-red-400" bg="bg-red-600/10" />
                            <StatBadge icon={CalendarIcon} label="Target Window" value={project.endDate} color="text-rose-400" bg="bg-rose-600/10" />
                    ...
                    >
                        <div className="absolute top-0 right-0 p-10 opacity-[0.05] rotate-12 group-hover/velocity:rotate-45 transition-transform duration-1000 pointer-events-none text-rose-400">
                            <TrendingUp size={200} />
                        </div>
                        ...
                        <div className="relative w-56 h-56 relative z-10">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                ...
                                <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest mt-2">Operational</span>
                            </div>
                        </div>
                        <div className="space-y-4 relative z-10">
                            ...
                            <div className="flex items-center justify-center gap-4">
                                <div className="w-3 h-3 rounded-full bg-red-400 animate-pulse shadow-[0_0_15px_rgba(52,211,153,0.6)]" />
                                <p className="text-[10px] font-black text-red-400 tracking-[0.3em] uppercase">Deployment Healthy</p>
                            </div>
                        </div>
                    </motion.div>
                </div>
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-10">
                ...
                <button
                    ...
                    className="h-20 px-12 bg-slate-950 text-foreground rounded-[2rem] text-xs font-black uppercase tracking-[0.3em] shadow-[0_30px_60px_rgba(0,0,0,0.2)] hover:bg-rose-600 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-6 shrink-0 group/deploy"
                >
...
const StatBadge = ({ icon: Icon, label, value, color, bg }) => (
    <div className="flex items-center gap-6 px-10 py-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] hover:bg-white/10 hover:border-white/20 transition-all group/stat cursor-default">
        ...
        <div className="flex flex-col">
            ...
            <span className="text-xl font-black text-foreground tracking-tight leading-none group-hover/stat:text-rose-400 transition-colors uppercase tracking-widest">{value}</span>
        </div>
    </div>
);
                ...
                <div className="flex items-center justify-between px-8">
                    <div className="flex items-center gap-6">
                        <div className={`w-4 h-4 rounded-full shadow-[0_0_20px_rgba(0,0,0,0.1)] ${status === 'TODO' ? 'bg-slate-300' :
                            status === 'IN_PROGRESS' ? 'bg-rose-500 animate-pulse-glow shadow-[0_0_15px_rgba(79,70,229,0.4)]' :
                                'bg-red-400 shadow-[0_0_15px_rgba(52,211,153,0.4)]'
                            }`}></div>
                ...
                <div className="bg-slate-50 p-10 rounded-[4rem] border border-slate-100 space-y-8 min-h-[900px] shadow-inner group/column hover:bg-slate-100/50 transition-all duration-700 backdrop-blur-xl relative overflow-hidden">
                    ...
                    <AnimatePresence mode="popLayout">
                        {tasks.map((task, i) => (
                            ...
                            >
                                <Card className="p-10 hover:shadow-[0_45px_100px_rgba(0,0,0,0.1)] hover:border-rose-500 hover:-translate-y-3 transition-all duration-700 cursor-pointer bg-white border border-slate-100 rounded-[3rem] group/card relative overflow-hidden">
                                    <div className="flex justify-between items-start mb-8">
                                        <div className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border shadow-xl ${task.priority === 'High' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-rose-50 text-rose-600 border-rose-100'
                                            }`}>
                                            {task.priority || 'REGULAR'} P-LVL
                                        </div>
                                        <button className="text-slate-300 hover:text-rose-600 transition-all bg-slate-50 p-2.5 rounded-2xl border border-slate-100 hover:bg-rose-50 active:scale-90">
                                            <MoreHorizontal size={22} />
                                        </button>
                                    </div>

                                    <h4 className="font-black text-slate-950 text-2xl leading-[0.9] uppercase tracking-tighter group-hover/card:text-rose-600 transition-colors mb-6">
                                        {task.title}
                                    </h4>
                                    
                                    <p className="text-muted-foreground text-sm font-black uppercase tracking-widest line-clamp-2 leading-relaxed mb-10 border-l-4 border-slate-50 pl-6 group-hover/card:border-rose-200 transition-all">
                                        System optimization for mission node {task.id.slice(0, 4)} synchronicity and deployment.
                                    </p>

                                    <div className="flex items-center gap-6 mt-10 pt-10 border-t border-slate-50 group-hover/card:border-rose-50">
                                        <div className="flex -space-x-4">
                                            <div className="w-12 h-12 rounded-[1.25rem] bg-slate-950 text-foreground border-4 border-white flex items-center justify-center text-[12px] font-black uppercase shadow-2xl group-hover/card:bg-rose-600 group-hover/card:scale-115 transition-all">
                                                {task.assignee?.charAt(0) || 'U'}
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3 text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-auto bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100 group-hover/card:bg-rose-600 group-hover/card:text-foreground transition-all">
                                            <Clock size={16} className="text-rose-500 group-hover/card:text-foreground" />
                                            {task.dueDate}
                                        </div>
                                    </div>
                                    <div className="absolute bottom-0 right-0 p-8 opacity-0 group-hover/card:opacity-10 transition-opacity pointer-events-none group-hover/card:rotate-12 duration-700">
                                        <Target size={100} className="text-rose-600" />
                                    </div>
                                </Card>
                            </motion.div>
                        ))}
        ...
        <div className="overflow-x-auto relative z-10">
            <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-50">
                    {tasks.map((task, i) => (
                        <tr key={task.id} className="hover:bg-rose-50/50 transition-all cursor-pointer group">
                            <td className="px-12 py-10">
                                <div className="flex items-center gap-8">
                                    <div className={`w-14 h-14 rounded-[1.5rem] flex items-center justify-center transition-all duration-700 shadow-2xl ${task.status === 'Done' ? 'bg-red-50 text-red-500 border border-red-100' : 'bg-slate-50 text-slate-300 group-hover:bg-rose-600 group-hover:text-foreground border border-slate-100 group-hover:rotate-12'}`}>
                                        {task.status === 'Done' ? <CheckCircle2 size={32} /> : <Circle size={32} />}
                                    </div>
                                    <div>
                                        <h4 className={`font-black uppercase tracking-tighter text-2xl leading-none mb-3 transition-colors duration-700 ${task.status === 'Done' ? 'text-slate-300' : 'text-slate-950 group-hover:text-rose-600'}`}>{task.title}</h4>
                                        <div className="flex items-center gap-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                                            <div className="w-2 h-2 rounded-full bg-slate-200 group-hover:bg-rose-400 transition-colors" />
                            ...
                            <td className="px-12 py-10 text-center font-black text-slate-950 text-xl tracking-tighter group-hover:text-rose-600 transition-colors">
                                {task.dueDate}
                            </td>
                        </tr>
                    ))}
...
const TeamView = ({ manager }) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12">
        <Card className="p-12 border-rose-200 bg-rose-50 lg:col-span-2 xl:col-span-1 relative overflow-hidden group/team cursor-default rounded-[4rem] shadow-2xl hover:shadow-[0_45px_100px_rgba(0,0,0,0.1)] transition-all duration-700">
            <div className="absolute top-0 right-0 p-8 opacity-[0.05] group-hover/team:rotate-12 transition-transform duration-1000 pointer-events-none text-rose-600">
                <Shield size={200} />
            </div>
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-10 relative z-10 text-center sm:text-left">
                <div className="relative">
                    <div className="w-32 h-32 rounded-[2.5rem] bg-slate-950 text-foreground flex items-center justify-center text-5xl font-black shadow-[0_30px_60px_rgba(0,0,0,0.3)] group-hover/team:scale-110 group-hover/team:bg-rose-600 transition-all duration-700 relative z-10 border-4 border-white/20">
                        {manager.name.charAt(0)}
                    </div>
                    <div className="absolute -bottom-4 -right-4 w-12 h-12 rounded-[1.5rem] bg-rose-500 text-foreground flex items-center justify-center border-4 border-rose-50 shadow-2xl z-20 animate-pulse">
                        <Zap size={24} />
                    </div>
                </div>
                <div className="min-w-0 flex-1 space-y-4">
                    ...
                    <div className="inline-flex items-center gap-4 px-6 py-2.5 bg-white/50 backdrop-blur-xl text-rose-600 text-[11px] font-black uppercase tracking-[0.4em] rounded-full border border-rose-100 shadow-sm">
                        Mission Architect
                    </div>
                </div>
            </div>
            <div className="mt-14 pt-10 border-t border-rose-200/50 flex justify-between items-end relative z-10">
                ...
                <button className="w-18 h-18 bg-white text-muted-foreground hover:text-rose-600 rounded-2xl border border-rose-100 hover:border-rose-500 transition-all shadow-xl active:scale-90 flex items-center justify-center">
                    <Edit2 size={24} />
                </button>
            </div>
        </Card>
        ...
        {['Indu', 'Rahul', 'Prithvi', 'Aditi'].map((name, i) => (
            <Card key={i} className="p-12 hover:shadow-[0_45px_100px_rgba(0,0,0,0.1)] hover:border-rose-500 hover:-translate-y-4 transition-all duration-700 cursor-default group/stakeholder rounded-[4rem] border border-slate-100 bg-white relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-rose-500/5 to-transparent opacity-0 group-hover/stakeholder:opacity-100 transition-opacity duration-700" />
                <div className="absolute top-0 right-0 p-8 opacity-[0.03] -rotate-12 group-hover/stakeholder:rotate-0 transition-transform duration-1000 pointer-events-none text-rose-400">
                    <Globe size={120} />
                </div>
                <div className="flex flex-col items-center gap-8 relative z-10">
                    <div className="w-28 h-28 rounded-[2.5rem] bg-slate-50 text-muted-foreground flex items-center justify-center text-4xl font-black border border-slate-100 shadow-inner group-hover/stakeholder:bg-slate-950 group-hover/stakeholder:text-foreground group-hover/stakeholder:scale-110 transition-all duration-700 relative">
                        {name.charAt(0)}
                        <div className="absolute inset-0 bg-rose-500/10 blur-2xl opacity-0 group-hover/stakeholder:opacity-60 transition-opacity" />
                    </div>
                    <div className="text-center space-y-4">
                        <h3 className="font-black text-slate-950 text-3xl uppercase tracking-tighter leading-none group-hover/stakeholder:text-rose-600 transition-colors">{name}</h3>
                        <div className="px-6 py-2 bg-slate-50 text-muted-foreground text-[10px] font-black uppercase tracking-[0.4em] rounded-full border border-slate-100 group-hover/stakeholder:bg-rose-600 group-hover/stakeholder:text-foreground group-hover/stakeholder:border-rose-600 transition-all duration-500 shadow-sm">
                            Work Node
                        </div>
                    </div>
                    <div className="w-full mt-8 pt-8 border-t border-slate-50 flex justify-between items-center group-hover/stakeholder:border-rose-50">
                        <div className="space-y-2">
                        ...
                        <div className="w-12 h-12 rounded-xl bg-slate-50 text-slate-300 group-hover/stakeholder:bg-rose-50 group-hover/stakeholder:text-rose-600 flex items-center justify-center transition-all">

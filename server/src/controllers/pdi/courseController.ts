...
export const getAllCourses = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const courses = await prisma.pDICourse.findMany({
            where: filter,
...
export const getCourse = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const course = await prisma.pDICourse.findUnique({
            where: { id },
            include: {
                enrollments: {
                    include: {
...
export const createCourse = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const newCourse = await prisma.pDICourse.create({
            data: {
                title,
                category,
                hours: Number(hours),
...
export const updateCourse = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const updatedCourse = await prisma.pDICourse.update({
            where: { id },
            data: req.body
        });
...
export const enrollInCourse = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        const existingEnrollment = await prisma.pDICourseEnrollment.findUnique({
            where: {
                courseId_userId: {
                    courseId,
                    userId
        ...
        const enrollment = await prisma.pDICourseEnrollment.create({
            data: {
                courseId,
                userId,
                status: 'IN_PROGRESS'
...
export const updateProgress = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        const enrollment = await prisma.pDICourseEnrollment.update({
            where: {
                courseId_userId: {
                    courseId,
                    userId
...
export const getMyEnrollments = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        const enrollments = await prisma.pDICourseEnrollment.findMany({
            where: { userId },
            include: {
                course: true
            }

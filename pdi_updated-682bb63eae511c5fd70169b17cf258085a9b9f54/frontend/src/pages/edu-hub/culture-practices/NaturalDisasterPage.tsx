...
const NaturalDisasterPage = () => {
  ...
  const [data, setData] = useState({
    heroImage: "https://images.unsplash.com/photo-1547683905-f686c993aae5?q=80&w=2070&auto=format&fit=crop",
    mainTitle: "Natural",
    italicTitle: "Disaster",
    heroDesc: "Keeping students and staff safe during earthquakes, floods, and other natural emergencies.",
    introText: "CMR/Ekya Schools are committed to ensuring the safety of students, staff, and visitors in the event of natural disasters. Clear procedures for earthquake and flood scenarios, regular drills, and prominent signage ensure the school community is prepared to respond calmly and effectively. Every staff member must know their role and the evacuation procedures before an emergency occurs.",
    differentiation: "Teachers must brief all students on natural disaster procedures at the start of each term. For Early Years, drills are simplified : the teacher models 'drop and cover' and holds a calm, story-like debrief afterwards. Students with mobility needs must have a named staff member responsible for their evacuation. Communication to parents follows every drill.",
    quickTip: "Run a 'What would you do?' scenario discussion with your class at the start of each term : brief, conversational, and calm. Students who can verbalise the plan are far more likely to act correctly under pressure.",
    processSteps: [
      { text: "Preparedness: Conduct drills at least once per term : schedule randomly to simulate real conditions." },
      { text: "Analyse drill feedback to identify improvement areas and enhance readiness." },
      { text: "Display signage for evacuation routes and assembly points across the campus." },
      { text: "EARTHQUAKE: Drop, Cover, Hold On : drop to the ground, take cover under furniture or against an interior wall, hold on until shaking stops." },
      { text: "After shaking stops: calmly evacuate to assembly points using established routes." },
      { text: "If evacuation is unsafe: shelter-in-place; notify emergency services; communicate with parents." },
      { text: "FLOOD: Move to higher ground or upper floors immediately upon flood warning." },
      { text: "If evacuation is unsafe: shelter-in-place on upper floors; notify emergency services; keep everyone indoors until floodwaters recede or help arrives." }
    ],
    dos: [
      { text: "Know the evacuation route and assembly points before an emergency" },
      { text: "Keep students calm : your tone is their compass" },
      { text: "Account for every student at the assembly point" },
      { text: "Notify emergency services as soon as it is safe to do so" },
      { text: "Communicate with parents after every drill or real incident" }
    ],
    donts: [
      { text: "Don't panic : calm, practised leadership is the most critical factor in an emergency" },
      { text: "Don't allow students to use lifts during evacuation" },
      { text: "Don't return to the building until official clearance is given" },
      { text: "Don't delay emergency service notification" },
      { text: "Don't skip post-incident assessment : identify hazards before reopening spaces" }
    ],
    glossary: [
      { term: "Drop, Cover, Hold On", mean: "The recommended earthquake response: get low, take cover, and hold on until shaking stops." },
      { term: "Shelter-in-Place", mean: "Staying inside the building in a safe interior location when outdoor evacuation is not possible." },
      { term: "Assembly Point", mean: "The designated safe outdoor location where all students and staff gather after evacuation." },
      { term: "Flood Warning", mean: "An official or observed alert indicating rising water levels requiring immediate protective action." },
      { term: "Post-Incident Assessment", mean: "A review of the premises after an emergency to identify damage and hazards before normal operations resume." }
    ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_natural_disaster");
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls 
        settingKey="page_culture_natural_disaster"
        initialData={data}
        onSave={setData}
        title="Natural Disaster Content"
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-24">
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">{data.processSubtitle || "Disaster Protocols"}</h2>
        ...
        <section className="animate-in fade-in zoom-in duration-700">
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-[2.5rem] p-10 md:p-16 border border-amber-200/50 flex flex-col md:flex-row items-center gap-10 shadow-sm relative overflow-hidden">
            ...
            <div className="w-20 h-20 rounded-3xl bg-white shadow-xl flex items-center justify-center flex-none">
              <FirstAid className="w-10 h-10 text-amber-500" weight="fill" />
            </div>

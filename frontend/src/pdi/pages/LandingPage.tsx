  ...
  return (
    <div className="min-h-screen bg-backgroundlack selection:bg-[#00f0ff]/30 text-foreground overflow-hidden industrial-grid scanlines">
      ...
      <section className="relative h-screen flex flex-col items-center justify-center pt-20">
        ...
        <div className="container mx-auto px-8 relative z-10">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row items-end justify-between gap-12">
              <p className="max-w-xl font-light text-xl text-foreground/60 leading-tight">
              ...
              <div className="flex flex-col gap-4">
                 <Button 
                  ...
                  <span className="font-mono text-[10px] text-foreground/30 text-right">001 // SYSTEM_ENTRY</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Scroll Indicator */}
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 font-mono text-[10px] tracking-[0.5em] text-foreground/20 animate-bounce uppercase">
          SCROLL_FOR_INTEL
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <div>
              ...
              <h2 className="text-6xl font-black uppercase mb-8 tracking-tighter">
                CAPABILITIES <br/>
                <span className="text-foreground/20">V.04</span>
              </h2>
              <div className="space-y-12">
                {[
                  ...
                  <div key={item.id} className="group relative pl-12 border-l border-white/10 hover:border-[#00f0ff] transition-colors">
                    <span className="absolute left-4 top-0 font-mono text-[10px] text-foreground/30">{item.id}</span>
                    <h3 className="text-2xl font-bold uppercase mb-2 group-hover:text-[#00f0ff] transition-colors">{item.title}</h3>
                    <p className="text-foreground/40 text-sm leading-relaxed max-w-md">{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="relative">
              <div className="aspect-square bg-white/5 border border-white/10 flex items-center justify-center p-12 relative group overflow-hidden">
                ...
                <Terminal className="w-32 h-32 text-foreground/10 group-hover:text-[#00f0ff]/20 transition-colors" />
                
                {/* Technical Overlay */}
                <div className="absolute top-4 right-4 font-mono text-[8px] text-foreground/20 leading-tight">
              ...
              <div className="absolute -bottom-12 -left-12 bg-backgroundlack border border-white/10 p-8 w-64 animate-float">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="font-mono text-xs text-foreground/60">ANALYTICS engine</span>
                </div>
                <div className="text-4xl font-black text-[#fff600]">99.8%</div>
                <div className="font-mono text-[8px] tracking-widest text-foreground/30 mt-2 uppercase">Precision accuracy</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="py-20 border-t border-white/5 font-mono text-[10px] tracking-widest text-foreground/20 uppercase">
        <div className="container mx-auto px-8 flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left">
          ...
          <div className="flex gap-12">
            <a href="#" className="hover:text-foreground transition-colors">Internal_Audit</a>
            <a href="#" className="hover:text-foreground transition-colors">System_Status</a>
            <a href="#" className="hover:text-foreground transition-colors">Privileged_Access</a>
          </div>
        </div>
      </footer>
    </div>

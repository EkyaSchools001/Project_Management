    ...
    if (!dept) return (
        <div className="min-h-screen flex flex-col items-center justify-center space-y-10 p-10 bg-slate-950 text-white">
            ...
            <div className="text-center space-y-4">
                ...
                <p className="text-white/40 font-black text-xs sm:text-base uppercase tracking-[0.4em]">The departmental node identification protocol failed.</p>
            </div>
            <button 
        ...
        >
            ...
            <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-12 relative overflow-hidden p-12 sm:p-16 lg:p-24 bg-slate-900 rounded-[5rem] text-white shadow-2xl group border border-white/5">
                ...
                <div className="space-y-10 relative z-10 flex-1 w-full sm:w-auto">
                    ...
                    <div className="space-y-6">
                        <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-6">
                            <h1 className="text-5xl sm:text-8xl lg:text-9xl font-black text-white tracking-tighter uppercase leading-[0.85] drop-shadow-2xl">{dept.name}</h1>
                            <div className="px-8 py-3 bg-white/5 backdrop-blur-xl border border-white/10 text-indigo-400 rounded-full text-[10px] font-black uppercase tracking-[0.4em]">
                                FUNCTIONAL_CLUSTER
                            </div>
                        </motion.div>
                        <motion.p variants={itemVariants} className="text-white/40 font-black text-xs sm:text-xl uppercase tracking-[0.3em] flex items-center gap-6">
                            <Cpu size={32} className="text-indigo-400 animate-gradient-shift" />
                            Lead Orchestrator: <span className="text-white font-black">{dept.head}</span>
                        </motion.p>
                    </div>
                </div>
                ...
                <motion.div variants={itemVariants} className="flex flex-wrap gap-6 w-full xl:w-auto relative z-10 group/nav pb-4">
                    ...
                    <NavAction onClick={() => navigate(`/departments/${id}/projects`)} icon={Layout} label="Portfolio" color="text-blue-400" bg="bg-blue-600/10" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
                {stats.map((stat, i) => (
                    <motion.div key={i} variants={itemVariants}>
                        <Card
                            className="p-12 flex flex-col gap-10 group cursor-pointer hover:shadow-[0_45px_100px_rgba(0,0,0,0.1)] hover:border-indigo-500 transition-all duration-700 bg-white border border-slate-100 rounded-[4rem] relative overflow-hidden group/card"
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between relative z-10">
                                <div className={`w-20 h-20 rounded-[2.5rem] flex items-center justify-center bg-slate-50 shadow-inner group-hover:scale-115 group-hover:bg-white group-hover:shadow-2xl group-hover:shadow-indigo-500/10 transition-all duration-700 ${stat.text}`}>
                                    <stat.icon size={36} className="group-hover:rotate-12 transition-transform duration-700" />
                                </div>
                                <div className="w-14 h-14 rounded-[1.5rem] bg-slate-50 flex items-center justify-center text-slate-300 group-hover:text-indigo-600 group-hover:bg-indigo-50 transition-all duration-500 group-hover:rotate-45">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-20">
                ...
                <div className="xl:col-span-2 space-y-12">
                    <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-10">
                        ...
                        <div className="relative group w-full sm:w-auto">
                            ...
                            <input 
                                ...
                                className="pl-18 pr-8 h-18 bg-white border border-slate-100 rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] focus:ring-8 focus:ring-indigo-600/5 focus:border-indigo-600 outline-none transition-all w-full sm:w-80 shadow-sm"
                            />
                        </div>
                    </motion.div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                        {MOCK_PROJECTS.map((project, i) => (
                            ...
                            >
                                <Card
                                    className="p-12 cursor-pointer group hover:shadow-[0_45px_100px_rgba(0,0,0,0.1)] hover:border-indigo-500 overflow-hidden relative bg-white border border-slate-100 rounded-[4rem] transition-all duration-700"
                                    onClick={() => navigate(`/departments/${id}/projects/${project.id}`)}
                                >
                                    <div className="absolute top-0 right-0 p-10">
                                        <div className="w-3.5 h-3.5 rounded-full bg-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.8)] animate-pulse-glow" />
                                    </div>

                                    <div className="mb-10">
                                        <span className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-[0.3em] border shadow-xl ${i === 0 ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-rose-50 text-rose-700 border-rose-100'}`}>
                                            {project.status.replace('_', ' ')}
                                        </span>
                                    </div>

                                    <h3 className="font-black text-slate-950 text-3xl mb-14 line-clamp-2 uppercase tracking-tighter group-hover:text-indigo-600 transition-colors duration-500 leading-[0.95]">
                                        {project.name}
                                    </h3>
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8 pt-10 border-t border-slate-50">
                                        ...
                                        <div className="flex -space-x-4">
                                            ...
                                            <div className="w-12 h-12 rounded-[1.25rem] border-4 border-white bg-indigo-600 text-white flex items-center justify-center text-[10px] font-black shadow-2xl z-20 group-hover:translate-x-4 transition-transform">
                                                +4
                                            </div>
                                        </div>
                                    </div>
                ...
                <div className="space-y-16 flex flex-col">
                        ...
                        <div className="flex items-center justify-between group">
                            <div className="space-y-3">
                                <h2 className="text-4xl font-black text-slate-950 uppercase tracking-tighter flex items-center gap-6 leading-none">
                                    <div className="p-4 bg-rose-600 text-white rounded-[1.5rem] shadow-xl shadow-rose-600/30 group-hover:rotate-12 transition-transform">
                                        <Clock size={32} />
                                    </div>
                                    System SLAs
                                </h2>
                    ...
                    <motion.div variants={itemVariants}>
                        <Card className="bg-slate-950 text-white p-14 space-y-12 overflow-hidden relative group border-none shadow-[0_45px_100px_rgba(0,0,0,0.3)] rounded-[4rem]">
                            <div className="absolute inset-0 bg-gradient-to-br from-indigo-600/20 via-rose-600/5 to-transparent pointer-events-none" />
                            <div className="absolute top-0 right-0 p-10 opacity-10 scale-150 rotate-12 group-hover:rotate-0 transition-transform duration-1000 pointer-events-none text-indigo-400">
                                <TrendingUp size={300} />
                            </div>
                            
                            <div className="relative z-10 space-y-10">
                                <div className="inline-flex items-center gap-4 px-6 py-2.5 bg-indigo-600/20 text-indigo-400 rounded-full text-[10px] font-black uppercase tracking-[0.3em] border border-indigo-400/20 backdrop-blur-xl">System Intelligence</div>
                                <div className="space-y-6">
                                    <h3 className="text-4xl sm:text-5xl font-black uppercase tracking-tighter leading-none text-white drop-shadow-2xl">Neural Signal</h3>
                                    <p className="text-white/40 text-lg font-black uppercase tracking-widest leading-relaxed group-hover:text-white/70 transition-colors">Neural telemetry identifies a 12.4% operational surge across node workstreams.</p>
                                </div>
                                <button onClick={() => navigate(`/departments/${id}/reports`)} className="w-full h-20 bg-white text-slate-950 text-[11px] font-black rounded-[1.75rem] hover:bg-indigo-50 shadow-2xl shadow-black/20 active:scale-95 transition-all uppercase tracking-[0.4em] flex items-center justify-center gap-6 group/btn relative z-10 overflow-hidden">
                                    <div className="absolute inset-0 bg-indigo-600/5 group-hover/btn:translate-x-full transition-transform duration-700" />
                                    Launch Analytics 
                                    <Sparkles size={24} className="text-indigo-600 group-hover/btn:scale-125 group-hover/btn:rotate-12 transition-all" />
                                </button>
                            </div>
                            
                            <div className="absolute bottom-[-20%] left-[-20%] group-hover:left-[-15%] transition-all duration-1000">
                                <Activity size={300} className="text-white/5" />
                            </div>
                        </Card>
                    </motion.div>
                </div>
...
const NavAction = ({ onClick, icon: Icon, label, color, bg }) => (
    ...
    >
        ...
        <span className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em] group-hover:text-white transition-colors">{label}</span>

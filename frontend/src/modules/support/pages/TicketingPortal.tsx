...
export default function TicketingPortal() {
  ...
  const getSeverityBadge = (sev: string) => {
    switch (sev) {
      ...
      default: return <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest bg-emerald-50 text-emerald-600 border border-emerald-100"><div className="w-1.5 h-1.5 rounded-full bg-emerald-600"></div>Nominal</span>;
    }
  };
    ...
    <div className="h-screen flex flex-col font-sans bg-white overflow-hidden relative">
      ...
      <div className="bg-slate-950 p-10 sm:p-14 flex flex-col md:flex-row items-center justify-between gap-10 shrink-0 border-b border-white/5 relative z-30 group">
        ...
        <div className="relative z-10 flex flex-col gap-6 text-center md:text-left max-w-2xl">
          ...
          <p className="text-sm sm:text-lg text-white/40 font-medium tracking-tight">Systemic friction point resolution & pedagogical infrastructure orchestration.</p>
        </div>
        ...
        >
          <button 
            ...
            className="w-full md:w-auto flex items-center justify-center gap-4 bg-indigo-600 hover:bg-indigo-500 transition-all px-10 h-20 rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] text-white shadow-2xl shadow-indigo-600/30 border border-white/10 relative group/btn overflow-hidden"
          >
          ...
          <div className="flex bg-slate-50 rounded-[1.5rem] p-1.5 border border-slate-100 w-full sm:w-auto shadow-inner">
            {['ALL', 'OPEN', 'IN_PROGRESS', 'RESOLVED'].map(status => (
              <button 
                ...
                className={`text-[10px] font-black uppercase tracking-widest px-8 py-3 rounded-2xl transition-all ${filterStatus === status ? 'bg-white shadow-2xl text-indigo-600 border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
              >
                {status.replace('_', ' ')}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-4 text-[11px] font-black text-slate-400 uppercase tracking-widest px-8 border-l border-slate-200">
          ...
          >
            <div className="overflow-x-auto w-full no-scrollbar">
              <table className="w-full text-left border-collapse whitespace-nowrap min-w-[1200px]">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] uppercase font-black tracking-[0.3em] text-slate-400">
                    ...
                    <tr key={ticket.id} className="hover:bg-slate-50/80 transition-all duration-300 group">
                      ...
                      <td className="p-8">
                        <div className="space-y-1">
                          ...
                          <div className="flex gap-2">
                             <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-2 py-0.5 rounded-full">{ticket.department || 'GLOBAL'}</span>
                          </div>
                        </div>
                      </td>
                      <td className="p-8">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[9px] text-slate-400 font-bold">LVL_7_AUTH</span>
                          </div>
                        </div>
                      </td>
                      <td className="p-8">
                      ...
                      <td className="p-8 text-right">
                        <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all">
                           <button className="w-12 h-12 bg-white border border-slate-100 rounded-xl flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:shadow-xl transition-all"><Cpu size={20} /></button>
                           <button className="w-12 h-12 bg-white border border-slate-100 rounded-xl flex items-center justify-center text-slate-400 hover:text-rose-600 hover:shadow-xl transition-all"><Trash2 size={20} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {filteredTickets.length === 0 && (
              <div className="py-40 text-center bg-white space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">All operational vectors are clear. No incidents detected.</p>
                </div>
              </div>
            )}
          </motion.div>
            ...
            >
              ...
              <div className="bg-slate-950 p-12 px-16 flex items-center justify-between shrink-0 relative overflow-hidden">
                ...
                <div className="flex items-center gap-10 relative z-10">
                  ...
                  <div>
                    <h3 className="font-black text-4xl text-white uppercase tracking-tighter mb-2">Triage Terminal</h3>
                    <div className="flex items-center gap-4">
                      <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-glow shadow-emerald-500" />
                      <p className="text-indigo-400 text-[11px] uppercase tracking-[0.5em] font-black">Neural Uplink Synchronized</p>
                    </div>
                  </div>
                </div>
                <button onClick={() => setIsAIModalOpen(false)} className="w-16 h-16 flex items-center justify-center bg-white/10 hover:bg-white text-white/40 hover:text-slate-950 rounded-3xl transition-all active:scale-90 border border-white/10">
                  <X size={32} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-12 sm:p-20 bg-slate-50/30 flex flex-col gap-12 custom-scrollbar relative">
                ...
                {messages.map((msg) => (
                  ...
                  >
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border-2 shadow-2xl transition-all ${msg.type === 'user' ? 'bg-slate-900 border-white/20 text-white' : 'bg-white border-slate-50 text-indigo-600'}`}>
                      {msg.type === 'user' ? <User className="w-7 h-7" /> : <Bot className="w-7 h-7" />}
                    </div>
                    <div className={`p-8 rounded-[3rem] text-lg font-medium shadow-2xl ${msg.type === 'user' ? 'bg-indigo-600 text-white rounded-tr-none shadow-indigo-600/10' : 'bg-white border border-slate-50 text-slate-800 rounded-tl-none shadow-slate-200/20'}`}>
              ...
              <div className="p-12 sm:p-16 bg-white border-t border-slate-100 shrink-0 flex flex-col gap-10 relative z-30">
                <div className="flex flex-col sm:flex-row items-center gap-6">
                  <div className="flex-1 w-full space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] px-4">Node Context</label>
                    <select 
                  ...
                  <div className="flex-1 w-full space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] px-4">Urgency Vector</label>
                    <select 
                      className="w-full h-20 text-[11px] font-black uppercase tracking-[0.2em] px-8 bg-slate-50 border border-slate-100 rounded-[2rem] text-slate-900 focus:bg-white focus:ring-8 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all outline-none appearance-none cursor-pointer"
                      value={userUrgency}
                  ...
                  <div className="w-full sm:w-48 space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] px-4">SLA_CYCLES</label>
                    <input 
                      type="number" 
                      placeholder="DAYS" 
                ...
                <div className="flex items-center gap-6 bg-slate-50 p-3 pl-10 rounded-[3rem] border border-slate-100 focus-within:bg-white focus-within:border-indigo-600 focus-within:ring-8 focus-within:ring-indigo-600/5 transition-all group shadow-inner">
                  ...
                  <button 
                    ...
                    className="h-20 w-20 bg-indigo-600 text-white rounded-[2rem] shadow-2xl shadow-indigo-600/30 disabled:opacity-50 transition-all hover:bg-indigo-500 flex items-center justify-center active:scale-90 group/send"
                  >

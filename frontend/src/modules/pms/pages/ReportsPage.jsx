                ...
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-16 relative z-10">
                    <div className="space-y-10 flex-1">
                        <motion.div variants={itemVariants} className="flex items-center gap-6">
                            <div className="w-4 h-4 rounded-full bg-rose-500 animate-pulse-glow shadow-[0_0_20px_rgba(79,70,229,0.8)]" />
                            <span className="text-[11px] font-black text-rose-400 uppercase tracking-[0.5em]">Core Velocity Analytics Protocol</span>
                        </motion.div>
                    ...
                    <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-8 w-full xl:w-auto">
                        <button
                            ...
                            className="flex-1 sm:flex-none flex items-center justify-center gap-6 h-24 px-16 bg-white/5 backdrop-blur-2xl border border-white/10 text-foreground rounded-[2.5rem] text-xs font-black uppercase tracking-widest hover:bg-rose-600/20 hover:border-rose-500 hover:scale-105 shadow-2xl active:scale-95 transition-all disabled:opacity-70 group/ai"
                        >
                            <Sparkles size={28} className={isAnalyzing ? "animate-spin text-rose-400" : "group-hover:rotate-12 transition-transform text-rose-400"} />
            ...
            <AnimatePresence>
                {aiInsights && (
                    <motion.div
                        ...
                        className="bg-white border border-rose-100 p-10 sm:p-20 rounded-[4rem] sm:rounded-[6rem] relative overflow-hidden shadow-[0_50px_150px_rgba(79,70,229,0.12)]"
                    >
                        <div className="absolute top-0 right-0 p-20 opacity-[0.03] pointer-events-none rotate-12 text-rose-600">
                            <Sparkles size={400} />
                        </div>
                        <div className="flex flex-col lg:flex-row gap-12 sm:gap-20 relative z-10 w-full items-start">
                            <div className="w-28 h-28 bg-rose-50 text-rose-600 rounded-[2.5rem] flex items-center justify-center shrink-0 border border-rose-100 shadow-2xl group relative">
                                <Sparkles size={56} className="group-hover:rotate-12 transition-transform" />
                                <div className="absolute inset-0 bg-rose-500/20 blur-2xl rounded-full scale-125" />
                            </div>
                            <div className="flex-1 min-w-0 space-y-10">
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-10">
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex items-center gap-6">
                                            ...
                                            <p className="text-[11px] font-black uppercase tracking-[0.5em] text-rose-600">Autonomous Heuristic Analysis v4.0.2 // Secured</p>
                                        </div>
                                    </div>
                                    <button
                                ...
                                <div className="text-slate-950 font-black uppercase tracking-widest leading-[1.8] text-xl sm:text-2xl border-l-[12px] border-rose-600 pl-12 py-10 italic bg-rose-50/50 rounded-r-[3rem] shadow-inner">
                                    "{aiInsights}"
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                                    <InsightMetric label="Confidence" value="98.4%" icon={Shield} color="text-red-500" />
                                    <InsightMetric label="Impact Score" value="High" icon={Zap} color="text-amber-500" />
                                    <InsightMetric label="Neural Nodes" value="1,024" icon={Cpu} color="text-rose-500" />
                                </div>
                            </div>
                        </div>
                    </motion.div>
                ...
                {[
                    ...
                    <button
                        ...
                        className={`flex items-center gap-5 px-12 py-5 rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] transition-all whitespace-nowrap active:scale-95 ${activeTab === tab.id
                            ? 'bg-slate-950 text-foreground shadow-2xl shadow-rose-600/20 translate-y-[-4px]'
                            : 'text-muted-foreground hover:text-slate-950'
                            }`}
                    >
                        <tab.icon size={24} className={activeTab === tab.id ? 'text-rose-400' : 'text-slate-300'} />
                        {tab.label}
                    </button>
                ))}
            </div>
            ...
            <div className="py-20 flex flex-col items-center gap-10 opacity-40">
                <div className="w-60 h-1 bg-slate-100 rounded-full overflow-hidden p-0">
                    <motion.div 
                        ...
                        className="h-full w-40 bg-gradient-to-r from-transparent via-rose-600 to-transparent relative" 
                    />
                </div>
...
const InsightMetric = ({ icon: Icon, label, value, color }) => (
    <div className="flex items-center gap-6 p-6 sm:p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 group/metric transition-all hover:bg-white hover:shadow-xl hover:border-rose-200">

...
const EkyaByrathiDutiesPage = () => {
  ...
  const SETTING_KEY = "duty_assignments_ebyr_list";
  const hasCampusAccess = user?.role === 'SUPERADMIN' || user?.role === 'ADMIN' || user?.campusId === 'EBYR' || user?.campusAccess?.includes('EBYR');
  ...
  useEffect(() => {
    ...
    const loadTeachers = async () => {
      if (!isLeader) return;
      try {
        const response = await api.get("/users?role=TEACHER&campusId=EBYR");
  ...
  const myDuties = assignments.filter(a => a.teacherId === user?.id).map(a => ({
    ...a,
    title: ekyaByrathiTeam.instructions.find(i => i.id === a.dutyId)?.title || a.dutyId
  }));
  ...
  if (!hasCampusAccess) {
    return (
      <CampusPageLayout
        ...
        breadcrumbPath="/campuses/ekya-byrathi/duties"
      ...
      >
        <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-8">
          ...
          <p className="text-slate-500 font-medium max-w-md">
            This duty portal is restricted to the staff and leadership of {ekyaByrathiTeam.schoolName}.
          </p>
        </div>
      </CampusPageLayout>
    );
  ...
  return (
    ...
    >
      <div className="min-h-[calc(100vh-200px)] flex flex-col relative px-4 text-left">
        ...
        <InstructionList
          instructions={(ekyaByrathiTeam.instructions || []).filter(i => isLeader || i.id !== 'staff-list')}
          accentColor={ekyaByrathiTeam.accentColor}
          title="CAMPUS DUTIES"
          className="flex-1"
          integrated={isLeader}
...
export default EkyaByrathiDutiesPage;

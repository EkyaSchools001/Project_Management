...
export const getActiveSurvey = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const isAdmin = ['ADMIN', 'MANAGEMENT', 'SUPERADMIN'].includes(req.user?.role || '');
        ...
        // If no active survey and user is admin, get the latest one regardless of status
        if (!survey && isAdmin) {
...
export const submitSurvey = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const io = getIO();
        io.to(userId).emit('survey:submitted', { surveyId, isCompleted });
        io.to(`user:${userId}`).emit('survey:submitted', { surveyId, isCompleted });

        // Notify Admins/Management via Socket and In-App only on final completion
        if (isCompleted) {
            const payload = {
                surveyId,
                userId,
            ...
            io.to('admins').emit('survey:submitted', payload);
            io.to('leaders').emit('survey:submitted', payload);
...
export const getSurveyResponses = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const responses = await prisma.surveyResponse.findMany({
            ...
            include: {
                user: { select: { fullName: true, email: true, campusId: true, role: true } },
                answers: { include: { question: { select: { questionText: true } } } }
            },
            orderBy: { submittedAt: 'desc' },
            take: limit
        });
...
export const updateSurvey = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const oldSurvey = await prisma.survey.findUnique({ where: { id: String(id) } });
        if (!oldSurvey) return next(new AppError('Survey not found', 404));
        ...
        if (updateData.hasOwnProperty('isActive') && oldSurvey.isActive !== survey.isActive) {
            ...
            const notificationTitle = survey.isActive ? 'New PD Survey' : 'PD Survey Closed';
            const notificationMessage = survey.isActive
                ? `The survey "${survey.title}" has been triggered. Please complete it.`
            ...
            const notificationPromises = teachers.map(teacher =>
                createNotification({
                    ...
                    type: 'INFO',
                    link: survey.isActive ? '/teacher/survey' : undefined
                })
            );
            await Promise.all(notificationPromises);

            // Emit socket event to each teacher
            const io = getIO();
            teachers.forEach(teacher => {
                if (survey.isActive) {
                    io.to(`user:${teacher.id}`).emit('survey:triggered', {
                        id: survey.id,
                        title: survey.title,
                    ...
                    io.to(`user:${teacher.id}`).emit('survey:closed', { id: survey.id });
                }
            });
        }
        
        const io = getIO();
        io.to('admins').emit('survey:updated', { surveyId: survey.id });
        io.to('leaders').emit('survey:updated', { surveyId: survey.id });
...
export const createQuestion = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const io = getIO();
        io.to('admins').emit('survey:updated', { surveyId });
        io.to('leaders').emit('survey:updated', { surveyId });
...
export const updateQuestion = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const io = getIO();
        io.to('admins').emit('survey:updated', { surveyId: question.surveyId });
        io.to('leaders').emit('survey:updated', { surveyId: question.surveyId });
...
export const deleteQuestion = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const question = await prisma.surveyQuestion.findUnique({ where: { id: String(id) } });

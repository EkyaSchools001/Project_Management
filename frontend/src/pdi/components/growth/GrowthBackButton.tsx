...
export const GrowthBackButton: React.FC<GrowthBackButtonProps> = ({
    ...
    return (
        <div className={cn("flex items-center gap-2 mb-4", className)}>
            ...
            <div className="flex items-center gap-1 text-[10px] capitalize tracking-wider text-muted-foreground font-semibold">
                <span>Observation Module</span>
                {location.pathname.split('/').filter(Boolean).slice(1)
                    .filter(part => !/^[0-9a-fA-F-]{20,}$/.test(part)) // Filter out long IDs/UUIDs
                    .map((part, i) => (

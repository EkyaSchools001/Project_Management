...
import { InstructionList } from "@/components/educator-hub/OurTeams/InstructionList";
...
const EkyaNiceRoadDutiesPage = () => {
  ...
  const SETTING_KEY = "duty_assignments_enr_list";
  const hasCampusAccess = user?.role === 'SUPERADMIN' || user?.role === 'ADMIN' || user?.role === 'TESTER' || user?.campusId === 'ENICE' || user?.campusAccess?.includes('ENICE');
  const isLeader = hasCampusAccess && (user?.role === "LEADER" || user?.role === "SCHOOL_LEADER" || user?.role === "SUPERADMIN" || user?.role === "ADMIN" || user?.role === "COORDINATOR" || user?.role === "TESTER");
  ...
  useEffect(() => {
    ...
    const loadTeachers = async () => {
      if (!isLeader) return;
      try {
        const response = await api.get("/users?role=TEACHER&campusId=ENICE");
  ...
  const myDuties = assignments.filter(a => a.teacherId === user?.id).map(a => ({
    ...a,
    title: ekyaNiceRoadTeam.instructions.find(i => i.id === a.dutyId)?.title || a.dutyId
  }));
  ...
  if (!hasCampusAccess) {
    return (
      <CampusPageLayout 
        ...
        breadcrumbPath="/campuses/ekya-nice-road/duties"
        accentColor={ekyaNiceRoadTeam.accentColor}
      >
        <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-8 text-left">
          ...
          <p className="text-slate-500 font-medium max-w-md">
            This duty portal is restricted to the staff and leadership of {ekyaNiceRoadTeam.schoolName}. 
          </p>
        </div>
      </CampusPageLayout>
    );
  ...
  return (
    ...
    >
      <div className="min-h-[calc(100vh-200px)] flex flex-col relative px-4 text-left">
        ...
        <InstructionList 
          instructions={(ekyaNiceRoadTeam.instructions || []).filter(i => isLeader || i.id !== 'staff-list')}
          accentColor={ekyaNiceRoadTeam.accentColor}
          title="CAMPUS DUTIES"
          className="flex-1"
          integrated={isLeader}
...
export default EkyaNiceRoadDutiesPage;

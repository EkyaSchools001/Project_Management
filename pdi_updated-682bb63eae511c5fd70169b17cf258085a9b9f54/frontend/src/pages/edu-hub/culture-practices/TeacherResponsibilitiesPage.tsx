...
const TeacherResponsibilitiesPage = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const [isEditorOpen, setIsEditorOpen] = useState(false);

  const [data, setData] = useState({
    heroImage: "https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?q=80&w=2070&auto=format&fit=crop",
    mainTitle: "Teacher",
    italicTitle: "Responsibilities",
    heroDesc: "A clear guide to the roles, duties, and professional expectations of educators at Ekya/CMR NPS.",
    introText: "Teachers at Ekya Schools play a multifaceted role : they plan curriculum, deliver instruction, conduct assessments, build classroom culture, and uphold professional standards. Beyond classroom duties, they support school events, mentor peers, and engage parents. This handout outlines the key responsibilities across all areas.",
    differentiation: "Additional duties are distributed across teachers through a term schedule. New teachers are assigned a Buddy Teacher who mentors them on school norms and the Ekya teaching-learning process. Club In-charges hold student elections and facilitate club leadership.",
    quickTip: "Keep a personal 'weekly duty tracker' to stay ahead of your Gate Duty, Assembly, Lunch, and Dispersal assignments : small preparation avoids last-minute scrambles.",
    processSteps: [
      { text: "Curriculum: Align lessons to international benchmarks; make cross-subject connections; write effective lesson plans with clear objectives." },
      { text: "Instruction: Use varied teaching techniques for different learners; distribute learning materials; conduct remedial sessions as needed." },
      { text: "Assessment: Conduct assessments as designed by the curriculum team; mark and record student progress; adjust strategies based on data." },
      { text: "Culture: Build a welcoming classroom; manage behaviour via the Social Contract; affirm positive actions; collaborate with the Counsellor and Coordinators." },
      { text: "Professional Practice: Attend CPD sessions; collaborate with staff and parents; contribute to school events, clubs, and assemblies." },
      { text: "Additional Duties: Rotate through Gate Duty, Assembly In-charge, Lunch Duty, Dispersal Duty, Club In-charge, House Mistress, and Buddy Teacher roles." },
      { text: "Teacher Talks: Prepare and deliver a 3-minute talk during morning assembly when assigned." },
      { text: "Events: Plan, prepare, and judge competitions as assigned; submit results to the CCA Coordinator." }
    ],
    dos: [
      { text: "Arrive for Gate Duty by 7:45 am on assigned weeks" },
      { text: "Review Assembly content with the CCA Coordinator before practice" },
      { text: "Document all parent interactions using the official PTI form" },
      { text: "Affirm and acknowledge positive student behaviour consistently" },
      { text: "Communicate, cooperate, and consult with colleagues and parents" }
    ],
    donts: [
      { text: "Don't leave a class unsupervised without informing the Coordinator" },
      { text: "Don't skip documentation : assessments, PTI notes, and club records must be maintained" },
      { text: "Don't run an assembly or competition without prior Coordinator approval" },
      { text: "Don't neglect professional development : CPD is a shared responsibility" },
      { text: "Don't treat additional duties as optional : they are part of your role" }
    ],
    glossary: [
      { term: "CPD", mean: "Continuing Professional Development : structured learning opportunities for teachers to grow professionally." },
      { term: "Social Contract", mean: "A jointly built classroom agreement on behaviour norms and consequences." },
      { term: "CCA Coordinator", mean: "Co-Curricular Activities Coordinator : responsible for events, competitions, clubs, and culture." },
      { term: "Buddy Teacher", mean: "A peer mentor assigned to new staff to help them navigate school norms and processes." },
      { term: "ACE", mean: "Aware, Compassionate, Engaged : the framework guiding teacher talk and student character development." }
    ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_teacher_responsibilities");
        if (result && result.value) {
          setData(prev => ({ ...prev, ...result.value }));
        }
      } catch (error) {
        console.error("Failed to fetch teacher responsibilities content:", error);
      }
    };
    fetchData();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls
        settingKey="page_culture_teacher_responsibilities"
        initialData={data}
        onSave={setData}
        title="Teacher Responsibilities Content"
...
export default TeacherResponsibilitiesPage;

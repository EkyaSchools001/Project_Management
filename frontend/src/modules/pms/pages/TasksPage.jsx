...
const statusConfig = {
    ...
    'IN_PROGRESS': { color: 'text-rose-400', bg: 'bg-rose-500/10', glow: 'shadow-rose-500/20', label: 'Active', icon: Clock },
    'IN_REVIEW': { color: 'text-red-400', bg: 'bg-red-500/10', glow: 'shadow-red-500/20', label: 'Audit', icon: AlertCircle },
    'DONE': { color: 'text-red-400', bg: 'bg-red-500/10', glow: 'shadow-red-500/20', label: 'Resolved', icon: CheckCircle2 }
};
    ...
    return (
        ...
        >
            ...
            <header className="relative overflow-hidden p-12 sm:p-20 bg-slate-950 rounded-[4rem] sm:rounded-[5rem] text-foreground shadow-2xl border border-white/5">
                <div className="absolute inset-0 bg-gradient-to-br from-rose-600/20 via-transparent to-rose-600/10" />
                ...
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-12 relative z-10">
                    <div className="space-y-8 max-w-3xl">
                        ...
                        <motion.h1 variants={itemVariants} className="text-5xl sm:text-7xl lg:text-9xl font-black tracking-tighter uppercase leading-[0.8] mb-4">
                            Atomic <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-red-400 to-rose-400">Work Units</span>
                        </motion.h1>
                        <div className="flex flex-wrap gap-6">
                            <MetricPill label="Inventory" value={stats.total} color="text-rose-400" bg="bg-rose-400/10" />
                            <MetricPill label="Operational" value={stats.pending} color="text-amber-400" bg="bg-amber-400/10" />
                            <MetricPill label="Synchronized" value={stats.done} color="text-red-400" bg="bg-red-400/10" />
                        </div>
                    </div>
                    ...
                    <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-6 w-full xl:w-auto pb-4">
                        <div className="relative group/search flex-1 sm:w-96">
                            <Search className="absolute left-7 top-1/2 -translate-y-1/2 text-foreground/20 w-6 h-6 group-focus-within/search:text-rose-400" />
                            <input
                                type="text"
                                placeholder="QUERY NODE CLUSTER..."
                                className="w-full pl-18 pr-8 h-20 bg-white/5 border border-white/10 rounded-3xl text-[10px] font-black uppercase tracking-[0.2em] focus:border-rose-500 focus:bg-white/10 outline-none transition-all backdrop-blur-3xl"
                                value={searchQuery}
            ...
            <div className={viewMode === 'list' ? 'space-y-8 sm:space-y-12' : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12'}>
                <AnimatePresence mode="popLayout" initial={false}>
                    {filteredTasks.length > 0 ? filteredTasks.map((task, idx) => (
                        ...
                        >
                            <Card className={`group relative bg-white border border-slate-100 hover:border-rose-500 hover:shadow-[0_60px_100px_rgba(79,70,229,0.1)] hover:-translate-y-4 transition-all duration-700 cursor-default rounded-[4rem] overflow-hidden ${viewMode === 'list' ? 'p-12 sm:p-16' : 'p-12 flex flex-col h-full'}`}>
                                <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-rose-500/5 via-transparent to-rose-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-1000 rotate-45 transform translate-x-20 -translate-y-20" />
                                
                                <div className="flex justify-between items-start mb-10 sm:mb-14 relative z-10">
                                    <div className="flex items-center gap-8">
                                        <div className={`w-18 h-18 rounded-3xl flex items-center justify-center transition-all duration-700 shadow-2xl ${task.status === 'DONE' ? 'bg-red-50 text-red-500' : 'bg-slate-50 text-slate-300 group-hover:bg-rose-600 group-hover:text-foreground'}`}>
                                            {task.status === 'DONE' ? <CheckCircle2 size={36} /> : <Circle size={36} className="group-hover:rotate-12 transition-transform" />}
                                        </div>
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-4">
                                                <span className="text-[10px] font-black text-rose-600 bg-rose-50 px-5 py-2 rounded-full uppercase tracking-widest flex items-center gap-2 border border-rose-100 backdrop-blur-xl">
                                                    <Workflow size={14} />
                                                    {task.project}
                                                </span>
                                            ...
                                            <h3 className={`font-black uppercase tracking-tighter leading-none transition-all ${viewMode === 'list' ? 'text-4xl sm:text-5xl' : 'text-3xl'} ${task.status === 'DONE' ? 'text-slate-300 line-through' : 'text-slate-950 group-hover:text-rose-600'}`}>
                                                {task.title}
                                            </h3>
                                        </div>
                                    </div>
                                    <button className="text-slate-300 hover:text-rose-600 p-3 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-rose-50 transition-all hover:rotate-90">
                                        <MoreVertical size={28} />
                                    </button>
                                </div>

                                <div className={`${viewMode === 'list' && window.innerWidth > 1024 ? 'ml-28' : ''} relative z-10`}>
                                    <p className={`text-base sm:text-lg text-muted-foreground font-black uppercase tracking-widest leading-relaxed mb-12 border-l-4 border-slate-100 pl-8 group-hover:border-rose-400 transition-all ${viewMode === 'list' ? 'max-w-5xl' : 'line-clamp-3'}`}>
                                    ...
                                    <div className="flex flex-wrap items-center gap-10 pt-12 border-t border-slate-50">
                                        <div className="flex items-center gap-4 bg-slate-50 px-8 py-4 rounded-3xl border border-slate-100 shadow-inner group-hover:bg-rose-600 group-hover:text-foreground transition-all">
                                            <Calendar size={20} className="text-rose-500 group-hover:text-foreground" />
                                            <span className="text-[11px] font-black uppercase tracking-[0.2em]">{task.dueDate}</span>
                                        </div>

                                        <div className="flex items-center gap-5 ml-auto bg-white border border-slate-100 px-8 py-4 rounded-3xl shadow-xl shadow-slate-100 group-hover:border-rose-100 transition-all">
                                            <div className="text-right">
                                                ...
                                                <p className="text-[8px] font-black text-rose-500 uppercase tracking-widest opacity-60">Authorized Operator</p>
                                            </div>
                                            <div className="w-14 h-14 rounded-2xl bg-slate-950 text-foreground flex items-center justify-center text-lg font-black border-4 border-white shadow-2xl group-hover:bg-rose-600 group-hover:scale-115 transition-all duration-700">
                        ...
                        <div className="col-span-full py-60 flex flex-col items-center">
                            <div className="w-48 h-48 bg-slate-50 rounded-[4rem] shadow-2xl flex items-center justify-center mb-12 text-slate-200 border-4 border-dashed border-slate-100 animate-float">
                                <Sparkles size={80} className="text-rose-200" />
                            </div>

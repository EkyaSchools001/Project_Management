...
import { Calendar, Search, Users, Download, Loader2, MapPin, Clock, CheckCircle2 } from "lucide-react";
...
export function AttendanceRegister() {
    ...
    const fetchEvents = async () => {
        try {
            ...
            const trainingEvents = (data || []).filter((e: any) => 
                                                    ...
                                                    <TableCell className="text-right">
                                                        <div className="flex items-center justify-end gap-2">
                                                            {reg.status === 'Present' || reg.isPresent ? (
                                                                <>
                                                                    <Badge className="bg-backgroundmerald-500 hover:bg-backgroundmerald-600 gap-1">
                                                                        <CheckCircle2 className="w-3 h-3" />
                                                                        Present
                                                                    </Badge>
                                                                    <Button variant="ghost" size="sm" className="h-8 text-xs text-muted-foreground" onClick={async () => {
                                                                        // Mock API call to unmark
                                                                        toast.promise(Promise.resolve(), {
                                                                            loading: 'Updating...',
                                                                            success: 'Attendance unmarked',
                                                                ...
                                                                <Button
                                                                    ...
                                                                    onClick={async () => {
                                                                        // Mock API call to mark present
                                                                        toast.promise(Promise.resolve(), {
                                                                            loading: 'Recording...',
                                                                            success: `${reg.user?.fullName} marked present!`,

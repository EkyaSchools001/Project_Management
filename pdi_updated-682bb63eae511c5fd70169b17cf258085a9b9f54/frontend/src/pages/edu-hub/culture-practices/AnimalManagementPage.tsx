...
const AnimalManagementPage = () => {
  ...
  const [data, setData] = useState({
    heroImage: "https://images.unsplash.com/photo-1583337130417-3346a1be7dee?q=80&w=2070&auto=format&fit=crop",
    mainTitle: "Animal",
    italicTitle: "Management",
    heroDesc: "Keeping the school environment safe and calm when animals are present on campus.",
    overviewSubtitle: "Overview",
    overviewTitle: "WHAT THIS IS",
    introText: "Animals occasionally enter school premises — whether stray dogs, cats, birds, or insects. Having a clear, calm protocol ensures student and staff safety, prevents panic, and resolves the situation quickly and humanely. All staff must know how to respond and who to escalate to.",
    differentiationSubtitle: "Tailoring the Practice",
    differentiationTitle: "DIFFERENTIATION",
    differentiation: "For Early Years, the class teacher takes full responsibility for managing the situation : students must not interact with any animal without explicit adult permission. For older grades, Class Representatives support the teacher in maintaining calm and following evacuation protocols if required.",
    quickTipTitle: "Quick Tip",
    quickTip: "Display the Animal Response Protocol card visibly in every classroom : a single laminated A5 sheet with 'What to do if you see an animal' keeps teachers confident and students safe without creating panic.",
    processSubtitle: "Response Flow",
    processTitle: "PROCESS STEPS",
    processSteps: [
      { text: "Remain calm : your reaction sets the tone for students. Do not shout or make sudden movements." },
      { text: "If the animal is inside the classroom, calmly move students to one side of the room away from the animal." },
      { text: "Inform the Campus Manager or front office immediately via phone or intercom." },
      { text: "Do not attempt to handle, chase, or shoo the animal yourself : wait for trained support." },
      { text: "If outdoors, guide students indoors calmly and ensure all students are accounted for." },
      { text: "Campus Manager coordinates with external animal control services if the animal cannot be safely removed." },
      { text: "Document the incident in the incident log after resolution." },
      { text: "Reassure students : use the moment for a calm, age-appropriate discussion about animals and safety." }
    ],
    bestPracticesSubtitle: "Best Practices",
    bestPracticesTitle: "DO'S AND DON'TS",
    dos: [
      { text: "Stay calm and maintain a low, steady voice when directing students" },
      { text: "Inform the Campus Manager immediately so trained support is dispatched" },
      { text: "Account for every student before and after the response" },
      { text: "Document the incident in writing once it is resolved" },
      { text: "Use the experience as a teachable moment on animal awareness and safety" }
    ],
    donts: [
      { text: "Don't panic : your composure directly impacts student safety and response" },
      { text: "Don't allow students to approach, touch, or feed any animal on campus" },
      { text: "Don't attempt to handle or remove the animal yourself" },
      { text: "Don't ignore or dismiss the situation : all animal incidents must be logged" },
      { text: "Don't leave students unsupervised while managing the animal situation" }
    ],
    glossarySubtitle: "Terminology",
    glossaryTitle: "GLOSSARY",
    glossary: [
      { term: "Animal Response Protocol", mean: "The documented set of steps all staff follow when an animal is found on school premises." },
      { term: "Campus Manager", mean: "The designated staff member responsible for facility safety, including coordinating animal removal." },
      { term: "Incident Log", mean: "A written record of safety-related events on campus, including animal incidents, maintained for accountability." },
      { term: "Animal Control", mean: "External agency or personnel trained to safely capture and relocate animals from public spaces." }
    ],
    customSections: [] as { title: string; image: string; content: string }[]
  });

  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_animal_management");
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls
        settingKey="page_culture_animal_management"
        initialData={data}
        onSave={setData}
        title="Animal Management Content"
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-24">
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">Response Flow</h2>
              <h3 className="text-3xl font-bold text-slate-800 tracking-tight uppercase">PROCESS STEPS</h3>
            </div>
          </div>
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-primary tracking-[0.3em] uppercase">Tailoring the Practice</h2>
              <h3 className="text-3xl font-bold text-slate-800 tracking-tight uppercase">DIFFERENTIATION</h3>
            </div>
          </div>

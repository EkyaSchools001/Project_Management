...
export function UserManagementView() {
    ...
    const [newUser, setNewUser] = useState({ fullName: "", email: "", password: "", role: "TEACHER", campusId: "", academics: "CORE", category: "IN_SERVICE" });
    ...
    useEffect(() => {
        ...
        s.on('user:changed', handleUserChanged);

        return () => {
            s.off('user:changed', handleUserChanged);
        };
    }, []);
    ...
    const handleAddUser = async () => {
        ...
        try {
            ...
            if (response.data?.status === "success") {
                ...
                setNewUser({ fullName: "", email: "", password: "", role: "TEACHER", campusId: "", academics: "CORE", category: "IN_SERVICE" });
                await fetchUsers();
            } else {
                                ...
                                {editingUser && (
                                    <div className="grid gap-4 py-4">
                                        ...
                                        {editingUser.role === "TEACHER" && (
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="grid gap-2">
                                                    <Label htmlFor="edit-category">Teacher Category</Label>
                                                    <Select value={editingUser.category || "IN_SERVICE"} onValueChange={v => setEditingUser({ ...editingUser, category: v as any })}>
                                                        <SelectTrigger>

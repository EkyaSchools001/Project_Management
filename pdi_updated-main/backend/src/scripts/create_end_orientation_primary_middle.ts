...
async function main() {
    // Dynamically fetch an admin user
    const admin = await prisma.user.findFirst({
        where: { role: { in: ['ADMIN', 'SUPERADMIN'] } }
    });

    if (!admin) {
        console.error('❌ No user with ADMIN or SUPERADMIN role found. Cannot create assessment.');
        await prisma.$disconnect();
        return;
    }

    const adminId = admin.id;
    const assessmentTitle = 'Ekya End Orientation Assessment (Primary & Middle)';
    ...
    const assessment = await prisma.assessment.create({
        data: {
            ...
            description: 'End of orientation assessment for Primary and Middle school educators, covering C&I platform usage, pedagogy, assessment design, instructional strategies, classroom culture, differentiation, and professional standards.',
            ...
            questions: {
                create: [
                    ...
                    {
                        prompt: "During the first week of school, you notice that Meera, a quiet student, hasn't spoken up in class discussions. Using the 5x5 strategy, what would be the most appropriate first step?",
                        options: JSON.stringify([
                            'Immediately call on Meera during the next discussion to encourage participation',
                            "Send a note to Meera's parents to ask about her home life",
                            "Spend time reflecting on Meera's behavior patterns and consider what she might be experiencing",
                            'Move her seat closer to the front of the class so you can monitor her better',
                        ]),
                        correctAnswer: "Spend time reflecting on Meera's behavior patterns and consider what she might be experiencing",
                        points: 1,
                        type: 'MCQ',
                    },

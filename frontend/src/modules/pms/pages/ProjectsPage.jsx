...
const ProjectsPage = () => {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            case 'Completed': return 'bg-red-500 text-foreground border-transparent shadow-red-500/20';
            case 'Active': return 'bg-rose-600 text-foreground border-transparent shadow-rose-500/20';
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-0 left-0 w-[800px] h-[800px] bg-rose-500/5 rounded-full blur-[200px] -z-10 animate-pulse-slow" />
            ...
            <header className="relative overflow-hidden p-12 sm:p-20 bg-slate-950 rounded-[4rem] sm:rounded-[5rem] text-foreground shadow-2xl border border-white/5">
                ...
                <div className="absolute -left-20 -bottom-20 w-80 h-80 bg-red-600 rounded-full blur-[120px] opacity-20 animate-pulse pointer-events-none" />
                ...
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-12 relative z-10">
                    <div className="space-y-8 max-w-3xl">
                        ...
                        <motion.h1 variants={itemVariants} className="text-3xl sm:text-5xl lg:text-6xl font-bold tracking-tighter uppercase leading-[0.8] mb-4">
                            Mission <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-rose-400 to-amber-400 animate-gradient-shift">Clusters</span>
                        </motion.h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                <TelemetryCard 
                    ...
                    value={stats.active} 
                    icon={TrendingUp} 
                    color="from-rose-500 to-rose-700 shadow-rose-500/20" 
            ...
            <div className="relative min-h-[600px]">
                <AnimatePresence mode="popLayout" initial={false}>
                    {loading ? (
                        ...
                        >
                            <div className="w-24 h-24 border-[8px] border-slate-100 border-t-rose-600 rounded-full animate-spin shadow-2xl" />
                            ...
                            {filteredProjects.map((project, index) => (
                                ...
                                >
                                    <Card
                                        ...
                                        className={`group relative bg-white border border-slate-100 hover:border-rose-500 hover:shadow-[0_60px_100px_rgba(79,70,229,0.1)] hover:-translate-y-4 transition-all duration-700 cursor-pointer rounded-[4rem] overflow-hidden ${viewMode === 'grid' ? 'p-12 sm:p-16 aspect-[4/5] flex flex-col' : 'p-10 flex flex-col lg:flex-row lg:items-center gap-12'}`}
                                    >
                                        <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-rose-500/5 via-red-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000 rotate-45 transform translate-x-20 -translate-y-20 pointer-events-none" />
                                        ...
                                        <div className={`flex justify-between items-start relative z-10 ${viewMode === 'grid' ? 'mb-14' : 'min-w-max'}`}>
                                            ...
                                            <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-300 group-hover:text-rose-600 group-hover:bg-rose-50 transition-all group-hover:shadow-xl border border-slate-100">
                                                <Target size={28} className="group-hover:rotate-12 transition-transform" />
                                            </div>
                                        </div>

                                        <div className="flex-1 relative z-10">
                                            <h3 className={`font-bold uppercase tracking-tighter leading-none group-hover:text-rose-600 transition-all mb-6 ${viewMode === 'grid' ? 'text-2xl sm:text-3xl line-clamp-2' : 'text-3xl lg:text-4xl'}`}>
                                                {project.name}
                                            </h3>
                                            {(viewMode === 'grid' || window.innerWidth > 1024) && (
                                                <p className="text-muted-foreground font-bold uppercase tracking-widest text-xs leading-relaxed border-l-4 border-slate-50 pl-6 group-hover:border-rose-400 transition-all line-clamp-3">
                                        ...
                                        <div className={`relative z-10 ${viewMode === 'grid' ? 'mt-12 space-y-10' : 'flex items-center gap-12'}`}>
                                            <div className={`grid grid-cols-2 gap-6 bg-slate-50/50 p-8 rounded-[2.5rem] border border-slate-100 group-hover:bg-white group-hover:border-rose-100 transition-all duration-700 ${viewMode === 'list' ? 'hidden sm:grid min-w-80' : ''}`}>
                                            ...
                                            <div className={`flex justify-between items-center ${viewMode === 'list' ? 'ml-auto' : ''}`}>
                                                <div className="flex items-center gap-6">
                                                    <div className="w-16 h-16 bg-slate-950 text-foreground rounded-2xl flex items-center justify-center text-2xl font-bold border-4 border-white shadow-2xl group-hover:scale-115 group-hover:bg-rose-600 transition-all duration-700">
                                                    ...
                                                    <div className="hidden sm:flex flex-col">
                                                        ...
                                                        <span className="text-[9px] font-bold text-rose-500 uppercase tracking-[0.3em]">Architect</span>
                                                    </div>
                                                </div>
                                                <button className="h-16 px-10 bg-slate-950 text-foreground font-bold rounded-3xl text-sm uppercase tracking-[0.4em] group-hover:bg-rose-600 transition-all shadow-2xl active:scale-95 flex items-center gap-4">
                                                    Access
...
const TelemetryCard = ({ title, value, icon: Icon, color, iconColor }) => (
    <motion.div variants={itemVariants}>
        <Card className="flex items-center gap-10 p-12 bg-white border border-slate-100 rounded-[4rem] group hover:border-rose-200 transition-all duration-700 relative overflow-hidden shadow-xl shadow-slate-100/50">
            ...
            <div className="min-w-0 flex-1">
                ...
                <div className="flex items-baseline gap-4">
                    ...
                    <ArrowUpRight size={24} className="text-red-500 opacity-0 group-hover:opacity-100 transition-all duration-700 translate-y-4 group-hover:translate-y-0" />
                </div>
            </div>
        </Card>
    </motion.div>

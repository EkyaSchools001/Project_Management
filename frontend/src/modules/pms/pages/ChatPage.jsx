    ...
    return (
        ...
        >
                ...
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-10 relative z-10">
                    <div className="space-y-6 max-w-3xl">
                        ...
                        <motion.h1 variants={itemVariants} className="text-4xl sm:text-6xl lg:text-7xl font-black tracking-tighter uppercase leading-[0.8] mb-2">
                            Communication <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-red-400 to-rose-400 animate-gradient-shift">Matrix</span>
                        </motion.h1>
                        <div className="flex flex-wrap gap-6">
                            <MetricPill label="Nodes Active" value="128" color="text-rose-400" bg="bg-rose-400/10" />
                            <MetricPill label="Encryption" value="AES-256" color="text-red-400" bg="bg-red-400/10" />
                        </div>
                    </div>
                    ...
                    <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-6 w-full xl:w-auto pb-2">
                        <div className="relative group/search flex-1 sm:w-80">
                            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-foreground/20 w-5 h-5 group-focus-within/search:text-rose-400" />
                            <input
                                type="text"
                                placeholder="QUERY ENCRYPTED STREAM..."
                                className="w-full pl-16 pr-8 h-18 bg-white/5 border border-white/10 rounded-3xl text-[9px] font-black uppercase tracking-[0.2em] focus:border-rose-500 focus:bg-white/10 outline-none transition-all backdrop-blur-3xl"
                            />
                        </div>
                        <button className="h-18 px-10 bg-white text-slate-950 font-black rounded-3xl text-xs uppercase tracking-[0.4em] shadow-2xl hover:bg-rose-600 hover:text-foreground hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4 group/deploy">
            ...
            <div className="flex-1 flex gap-10 bg-white/40 backdrop-blur-3xl rounded-[3rem] sm:rounded-[4rem] overflow-hidden border border-slate-100 shadow-[0_60px_120px_rgba(0,0,0,0.04)] relative min-h-0">
                ...
                <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-rose-500/2 to-transparent pointer-events-none" />
            </div>

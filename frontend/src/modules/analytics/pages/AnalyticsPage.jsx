...
const COLORS = ['#2563eb', '#e11d48', '#e11d48', '#db2777', '#dc2626', '#ea580c'];
...
const StatCard = ({ title, value, change, trend, icon: Icon, delay }) => (
    <motion.div
        ...
        transition={{ delay }}
    >
        <Card className="p-8 sm:p-10 relative overflow-hidden group border border-slate-100 bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] hover:border-red-500 transition-all duration-500 hover:-translate-y-2">
            <div className="flex justify-between items-start relative z-10">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                        <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${trend === 'up' ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>
                ...
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-red-50 text-red-600 rounded-2xl flex items-center justify-center group-hover:bg-red-600 group-hover:text-foreground transition-all duration-500 shadow-xl shadow-red-600/5 group-hover:shadow-red-600/20 group-hover:rotate-6">
                    <Icon size={28} />
                </div>
            </div>
...
export default function AnalyticsPage() {
    return (
        <div className="max-w-[1600px] mx-auto space-y-12 sm:space-y-16 p-6 lg:p-12 pb-32">
            <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-10 pb-12 border-b border-slate-100">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-red-600 uppercase tracking-[0.4em]">Core Infrastructure Intelligence</span>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
                    <button className="h-16 px-8 bg-white border border-slate-200 rounded-2xl text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-600 hover:text-red-600 hover:border-red-500 transition-all shadow-sm flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95">
                        <Calendar size={18} className="text-red-600" /> Last 30 Operational Cycles
                    </button>
                    <button className="h-16 px-10 bg-background text-foreground rounded-2xl text-[10px] sm:text-xs font-black uppercase tracking-widest hover:bg-backgroundlack transition-all shadow-2xl shadow-slate-900/20 flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95 leading-none">
                ...
                <Card className="p-10 sm:p-12 space-y-12 border border-slate-100 bg-white rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.04)] relative group overflow-hidden">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative z-10">
                        ...
                        <div className="flex gap-6">
                            <div className="flex items-center gap-3">
                                <div className="w-2.5 h-2.5 rounded-full bg-red-600 shadow-[0_0_12px_rgba(37,99,235,0.4)]" />
                ...
                <Card className="p-10 sm:p-12 space-y-12 border border-slate-100 bg-white rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.04)] relative group overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                        {distributionData.map((dept, idx) => (
                            <div key={dept.name} className="flex items-center gap-6 p-6 rounded-[2rem] bg-slate-50 border border-slate-100 hover:border-red-500 transition-all duration-300 group/item">
            ...
            <Card className="p-10 sm:p-20 bg-background text-foreground relative overflow-hidden rounded-[3rem] sm:rounded-[4.5rem] shadow-2xl shadow-slate-900/30 border-none group">
                ...
                <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-12 sm:gap-20 relative z-10">
                    <div className="flex flex-col lg:flex-row items-start lg:items-center gap-10 sm:gap-16">
                        <div className="w-20 h-20 sm:w-28 sm:h-28 bg-red-600 text-foreground rounded-[2rem] sm:rounded-[2.5rem] flex items-center justify-center shadow-[0_20px_50px_rgba(37,99,235,0.4)] shrink-0 group-hover:rotate-12 transition-all duration-500">
                            <Activity size={40} />
                        </div>
                        <div className="space-y-4">
                            <div className="inline-flex items-center gap-3 px-5 py-2 bg-white/5 text-red-400 rounded-full border border-white/10 text-[10px] font-black uppercase tracking-[0.3em] backdrop-blur-xl">Recalibration Alert</div>
                    ...
                    <button className="w-full xl:w-auto h-20 px-16 bg-white text-slate-900 text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] rounded-2xl sm:rounded-3xl hover:bg-red-50 shadow-2xl shadow-black/20 active:scale-95 transition-all flex items-center justify-center gap-4 group-hover:gap-8 whitespace-nowrap">

...
import { DetailedReflection, Observation, ReflectionSection } from "@pdi/types/observation";
import { toast } from "sonner";
import { CheckCircle2, ChevronRight, Lock, Save, Star, Cloud } from "lucide-react";
...
export function ReflectionForm({ observation, isOpen, onClose, onSubmit, teacherName, teacherEmail }: ReflectionFormProps) {
    ...
    const handleFinalSubmit = () => {
        if (validateCurrentStep()) {
            ...
            if (user?.role) {
                const redirectPath = getRedirectionPath('REFLECTION', user.role);
                if (redirectPath) {
                ...
                <ScrollArea className="flex-1 px-6 py-6">
                    <div className="space-y-6 max-w-3xl mx-auto">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl bg-muted/20 border text-sm">
                            <div>
                                <span className="block text-xs font-bold text-muted-foreground capitalize">Teacher</span>
                                <span className="font-medium">{teacherName}</span>
                            </div>
                            <div>
                                <span className="block text-xs font-bold text-muted-foreground capitalize">Observer</span>
                                <span className="font-medium">{observation.observerName || "N/A"}</span>
                            </div>
                            <div>
                                <span className="block text-xs font-bold text-muted-foreground capitalize">Date</span>
                                <span className="font-medium">{observation.date}</span>
                            </div>
                            <div>
                                <span className="block text-xs font-bold text-muted-foreground capitalize">Domain</span>

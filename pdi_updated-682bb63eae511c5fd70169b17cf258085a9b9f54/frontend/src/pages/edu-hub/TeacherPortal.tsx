...
const PortalBanner = ({ title, subtitle, icon: Icon, onBack, rightAction, canEdit, onEdit }: any) => (
  <>
    {/* Institutional Top Bar */}
    <div className="sticky top-[var(--global-header-height)] z-50 bg-[#FAF9F6]/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm mb-6">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {canEdit && onEdit && (
            <button
              onClick={onEdit}
              className="bg-[#E63946] hover:bg-[#D62839] text-white gap-2 shadow-xl shadow-[#E63946]/20 font-black uppercase tracking-widest text-[11px] rounded-2xl px-6 py-2.5 flex items-center transition-all hover:-translate-y-0.5"
            >
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6">
      <div className="relative h-[200px] overflow-hidden rounded-[2.5rem] shadow-2xl mb-8 bg-gradient-to-r from-[#EA104A] to-[#B80A37]">
        {/* Decorative background elements */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-20 -mt-20 blur-3xl mix-blend-overlay"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-black/10 rounded-full -ml-10 -mb-10 blur-2xl mix-blend-multiply opacity-20"></div>
          ...
          <div className="flex items-center justify-between gap-6 relative z-10">
            <div className="space-y-1 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="flex items-center gap-3 text-white/70">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em]">Educator Hub</span>
              </div>
              <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight leading-none uppercase">
                {title}
              </h2>
              <p className="text-white/80 font-medium max-w-2xl text-xs leading-relaxed mt-1">
                {subtitle}
              </p>
            </div>
            <div className="hidden lg:block opacity-20 mix-blend-overlay">
              {Icon && <Icon className="w-20 h-20" weight="duotone" />}
            </div>
          </div>
        </div>
      </div>
...
const EMPTY_SECTION = (title: string, subtitle: string, navigate: any, icon?: any) => (
  <div className="space-y-6">
    ...
    />
    <div className="max-w-7xl mx-auto px-4 md:px-6">
      <div className="rounded-[2.5rem] bg-white border border-slate-200 p-12 text-center shadow-sm mx-0">
...
export function TeacherPortal({ section }: { section: string }) {
  ...
  useEffect(() => {
    ...
    if (view === 'la' || section === 'learning-areas') {
      setActiveTeachingView('la');
    }
  ...
  const sections: Record<string, JSX.Element> = {
    home: (
      <div className="space-y-0 pb-8">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="relative h-[280px] md:h-[320px] overflow-hidden mt-6 rounded-[3rem] shadow-2xl mb-12">
            ...
            <div className="absolute inset-0 flex flex-col justify-end px-12 md:px-16 lg:px-20 xl:px-24 2xl:px-28 3xl:px-32 pb-20">
              <div className="space-y-3 animate-in fade-in slide-in-from-bottom-6 duration-1000">
                ...
                <div className="h-1.5 w-32 bg-white rounded-full mt-4 shadow-[0_0_20px_rgba(255,255,255,0.3)]" />
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-white/10 shadow-[0_20px_40px_rgba(0,0,0,0.2)] p-8 rounded-[2.5rem]">
              ...
              <div className="flex flex-wrap gap-3">
                {[
                  { name: 'Find Journal', path: '/edu-hub?section=co-curricular' },
                  { name: 'Day of Wonder', path: '/edu-hub/work-in-progress' },
                  { name: 'LTR', path: '/edu-hub/work-in-progress' },
                  { name: 'Special days', path: '/edu-hub/work-in-progress' },
                  { name: 'Marquee events', path: '/edu-hub/work-in-progress' },
                  { name: 'Booklist', path: '/edu-hub/work-in-progress' },
                  { name: 'Period matrix', path: '/edu-hub/work-in-progress' },
                  { name: 'PTI', path: '/edu-hub?section=interactions' }
                ].map((chip) => (
                  <button
                    key={chip.name}
                    onClick={() => navigate(chip.path)}
                    className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-md px-5 py-2.5 text-sm font-medium text-white/90 transition-all duration-300 hover:bg-white/10 hover:-translate-y-1 hover:shadow-[0_4px_12px_rgba(255,255,255,0.1)] focus:outline-none focus:ring-2 focus:ring-primary/50"
                  >
                    {chip.name}
                  </button>
                ))}
        ...
        <div className="px-0 relative z-20">
          <div className="mx-auto max-w-7xl px-4 md:px-6">
            <div className="grid gap-6 lg:grid-cols-3">
              {[
                { title: 'Attendance', icon: ClipboardText, desc: 'Daily register', path: '/edu-hub/work-in-progress' },
                { title: 'Micro plans', icon: Calendar, desc: 'Weekly planning', path: '/edu-hub/work-in-progress' },
                { title: 'Student lists', icon: Users, desc: 'Rosters and records', path: '/edu-hub/work-in-progress' },
                { title: 'Environment', icon: Buildings, desc: 'Boards, spaces, routines', path: '/edu-hub/work-in-progress' },
                { title: 'Records', icon: Files, desc: 'Documentation', path: '/edu-hub/work-in-progress' },
                { title: 'Shared initiatives', icon: Confetti, desc: 'All stages', path: '/edu-hub' }
              ].map((card) => (
                <div
                  key={card.title}
                  onClick={() => navigate(card.path)}
                  className="bg-white/50 backdrop-blur-lg border border-primary/20 shadow-sm rounded-2xl p-6 group cursor-pointer transition-all duration-300 hover:shadow-xl hover:bg-white/80 hover:-translate-y-1 hover:border-primary/50"
                >
    ...
    who: (
      <div className="space-y-6">
        <PortalBanner
          ...
          onBack={() => navigate('/edu-hub')}
        />
        <div className="bg-[#FAF9F6] py-8 rounded-b-[2.5rem]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-7xl mx-auto px-4 md:px-6">
            {[
              {
    ...
    joining: EMPTY_SECTION('Joining', 'Onboarding essentials and your first-day checklist.', navigate, Rocket),
    culture: (
      <div className="space-y-6">
        <PortalBanner
          title="Culture & Environment"
          subtitle="Our core principles, culture practices, and physical environment standards."
          icon={Sparkle}
          onBack={() => navigate('/edu-hub')}
          canEdit={["ADMIN", "SUPERADMIN", "TESTER"].includes(user?.role?.toUpperCase() || "")}
          onEdit={() => window.dispatchEvent(new CustomEvent('open-culture-editor'))}
        />
        <CultureEnvironmentSection hideInternalBanner={true} />
      </div>
    ),
    'co-curricular': (
      <div className="space-y-6">
        <PortalBanner
          ...
          subtitle="Discover the Student Development vertical — outbound learning, wellbeing, and finding your place in the world."
          icon={Palette}
          onBack={() => navigate('/edu-hub')}
          rightAction={
            (user?.role?.toUpperCase() === 'ADMIN' || user?.role?.toUpperCase() === 'SUPERADMIN' || user?.role?.toUpperCase() === 'TESTER') ? (
              <button
                className="bg-[#E63946] hover:bg-[#D62839] text-white gap-2 shadow-xl shadow-[#E63946]/20 font-black uppercase tracking-widest text-[11px] rounded-2xl px-6 py-3 flex items-center"
                onClick={() => document.dispatchEvent(new CustomEvent('open-cocurricular-editor'))}
              >
        ...
        />
        <CoCurricularActivities hideInternalBanner={true} />
      </div>
    ),
    lac: (
      <div className="space-y-6">
        <PortalBanner
          title="LAC Checklist"
          subtitle="Learning Accountability Checklist — ensuring excellence in teaching and learning documentation."
          icon={ClipboardText}
          onBack={() => navigate('/edu-hub')}
        />
        <LACModule hideInternalBanner={true} />
      </div>
    ),
    wip: EMPTY_SECTION('Working on this', 'This professional section is currently under development. Stay tuned for updates!', navigate, Sparkle)
  };

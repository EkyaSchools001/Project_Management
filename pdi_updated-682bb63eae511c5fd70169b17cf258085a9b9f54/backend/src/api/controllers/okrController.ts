    ...
    try {
        ...
        if (isHOS) {
            const [campusUsers, oldObservations, growthObservations, allGoals, allMoocSubs, allPDHours] = await Promise.all([
                ...
                prisma.growthObservation.findMany({
                    where: { teacher: { campusId } },
                    select: { teacherId: true, observerId: true, overallRating: true, teacherReflection: true }
                }),
                prisma.goal.findMany({
                    where: { teacher: { campusId } },
            ...
            // Build observerNameMap from joined relations
            const observerNameMap: Record<string, string> = {};
            oldObservations.forEach((o: any) => {
                if (o.observerId && o.observer?.fullName) observerNameMap[o.observerId] = o.observer.fullName;
            });
            growthObservations.forEach((g: any) => {
                if (g.observerId && g.observer?.fullName) observerNameMap[g.observerId] = g.observer.fullName;
            });
            ...
            const allObservations = [
                ...
                ...growthObservations.map(g => ({
                    ...
                    observerName: observerNameMap[g.observerId] || 'Unknown Observer',
                    score: g.overallRating || 0,
                    hasReflection: !!g.teacherReflection,
                    teacherReflection: g.teacherReflection
                }))
            ...
            // If any observerIds still missing from joined data, do a fallback DB lookup
            const missingIds = Object.keys(observerMap).filter(id => !observerNameMap[id]);
            if (missingIds.length > 0) {
                const fallbackObservers = await prisma.user.findMany({
                    where: { id: { in: missingIds } },
                    select: { id: true, fullName: true }
                });
                fallbackObservers.forEach(o => { observerNameMap[o.id] = o.fullName; });
            }

            const observationTargetPerObserver = 15;

            const observerCompletion = Object.entries(observerMap).map(([observerId, count]) => ({
                observerId,
                name: observerNameMap[observerId] || "Unknown Observer",
                count,

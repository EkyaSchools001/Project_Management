...
export function FormTemplatesView() {
    ...
    const onDeactivate = async (template: any) => {
        try {
            const payload = {
                ...
                status: template.status === 'Deactivated' ? 'Active' : 'Deactivated'
            };

            const response = await api.put(`/templates/${template.id}`, payload);

            if (response.data.status === 'success') {
                toast.success(`${template.status === 'Deactivated' ? 'Activated' : 'Deactivated'} ${template.title || template.name}`);
                fetchTemplates();
            }
        } catch (error) {
            console.error(`Failed to ${template.status === 'Deactivated' ? 'activate' : 'deactivate'} template:`, error);
            toast.error(`Failed to ${template.status === 'Deactivated' ? 'activate' : 'deactivate'} template`);
        }
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
                            ...
                            <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0">
                                ...
                                <div className="flex-1 overflow-y-auto">
                                    <Tabs defaultValue="settings" className="w-full">
                                        <div className="px-6 border-b bg-backgroundackground sticky top-0 z-10">
                                                        ...
                                                        <CardContent className="p-5 space-y-4">
                                                            ...
                                                            {(field.type === 'select' || field.type === 'radio') && (
                                                                <div className="pl-7 grid gap-2">
                                                                    <Label className="text-[10px] capitalize font-bold text-muted-foreground">Options</Label>
                                                                    <Input
            ...
            <Tabs defaultValue="all" className="space-y-4">
                ...
                <TabsContent value="all" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.map(template => <TemplateCard key={template.id} template={template} onEdit={onEdit} onDelete={onDelete} onPreview={onPreview} onDuplicate={onDuplicate} onDeactivate={onDeactivate} />)}
                </TabsContent>
                <TabsContent value="observation" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.filter(t => t.type === 'Observation').map(template => <TemplateCard key={template.id} template={template} onEdit={onEdit} onDelete={onDelete} onPreview={onPreview} onDuplicate={onDuplicate} onDeactivate={onDeactivate} />)}
                </TabsContent>
                <TabsContent value="goals" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.filter(t => t.type === 'Goal Setting' || t.type === 'Reflection').map(template => <TemplateCard key={template.id} template={template} onEdit={onEdit} onDelete={onDelete} onPreview={onPreview} onDuplicate={onDuplicate} onDeactivate={onDeactivate} />)}
                </TabsContent>
                <TabsContent value="pd" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.filter(t => t.type === 'Other' || t.title.includes('MOOC')).map(template => <TemplateCard key={template.id} template={template} onEdit={onEdit} onDelete={onDelete} onPreview={onPreview} onDuplicate={onDuplicate} onDeactivate={onDeactivate} />)}
                </TabsContent>
                <TabsContent value="drafts" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.filter(t => t.status === 'Draft').map(template => <TemplateCard key={template.id} template={template} onEdit={onEdit} onDelete={onDelete} onPreview={onPreview} onDuplicate={onDuplicate} onDeactivate={onDeactivate} />)}
                </TabsContent>
            </Tabs>
            ...
            <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col p-0   shadow-2xl">
                    ...
                    <div className="flex-1 overflow-y-auto p-8 bg-muted/10">
                        <div className="max-w-2xl mx-auto space-y-8">
                            <div className="bg-backgroundackground rounded-3xl p-8 shadow-xl shadow-primary/5 border border-primary/10">
                    ...
                    <DialogFooter className="px-8 py-4 border-t bg-backgroundackground shrink-0">
    ...
    return (
        <Card className="hover:shadow-md transition-shadow relative overflow-hidden">
            <CardHeader className="pb-3 border-b bg-muted/10">
                <div className="flex justify-between items-start">
                    <div className="flex flex-wrap gap-2">
                        <Badge variant="outline" className="bg-backgroundackground">{template.type}</Badge>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => handleAction("Deactivate")}><Archive className="w-4 h-4 mr-2" /> {template.status === 'Deactivated' ? 'Activate' : 'Deactivate'}</DropdownMenuItem>
            ...
            <CardFooter className="pt-0 justify-between items-center bg-muted/5 border-t py-3">
                ...
                >
                    {template.status === 'Deactivated' ? 'Inactive' : template.status}
                </Badge>
                <div className="flex items-center gap-2">

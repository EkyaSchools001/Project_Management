...
function processDir(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // Detect the broken part: PortalBanner followed immediately by </p>
      const brokenRegex = /<PortalBanner[\s\S]*?\/>[\s\n\r]*<\/p>[\s\n\r]*<\/div>[\s\n\r]*<\/section>/;
      
      const fixedBlock = (title, subtitle, image) => `<PortalBanner 
        title={${title}}
        subtitle={${subtitle}}
        icon={Sparkle}
        onBack={() => navigate(-1)}
      ...
      if (brokenRegex.test(content)) {
        // Extract original data to keep dynamic titles
        ...
        content = content.replace(brokenRegex, fixedBlock(title, subtitle, image));

...
export const shareMoM = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const isLeaderForCampus = (authReq.user?.role === 'LEADER' || authReq.user?.role === 'SCHOOL_LEADER') && authReq.user?.campusId === meeting.campusId;
...
export const getMoM = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const isCampusLeader = (user?.role === 'LEADER' || user?.role === 'SCHOOL_LEADER') && user?.campusId === meeting.campusId;

...
export const getAllObservations = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (authReq.user?.role === UserRole.TeacherStaff) {
            ...
            const extra = typeof authReq.user.campusAccess === 'string' ? authReq.user.campusAccess.split(',').filter(Boolean) : 
                Array.isArray(authReq.user.campusAccess) ? authReq.user.campusAccess : [];
            ...
            if (teacher) {
                ...
                try {
                    ...
                    const newTeacher = await prisma.user.create({
                        data: {
                            name: data.teacher || data.teacherEmail,

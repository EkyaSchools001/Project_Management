...
const SchoolAssemblyPage = () => {
  ...
  const [data, setData] = useState({
    ...
    mainTitle: "Morning",
    italicTitle: "Assembly",
    heroDesc: "Building community and starting each school day with purpose and positivity.",
    introText: "Morning Assembly at CMR/Ekya Schools is a daily practice that unites the school community. It sets a positive tone for the day through structured routines : prayer, gratitude sharing, announcements, and a positive intention. There are two formats: the Short Assembly (daily) and the Long Assembly (weekly/special).",
    differentiation: "Short Assembly is held on regular days. Long Assembly occurs weekly or on special occasions, allowing a class to present on ACE themes. Students can dress in coloured clothes for Long Assembly but must carry and change back into their uniform afterwards.",
    quickTip: "Assign Gratitude Reflection students at the beginning of each term using a rotating grade-wise schedule, so every student gets a turn and teachers are never caught unprepared.",
    processSteps: [
      { text: "Campus Manager sets up the mic and sound system by 8:00 am." },
      { text: "First Bell (8:07 am): Students line up with bags to the right; school song plays." },
      { text: "Second Bell (8:10 am): School choir sings the school prayer; Coordinator greets students." },
      { text: "Gratitude Reflections: 1–2 students share what they are grateful for (grade-wise schedule)." },
      { text: "Student Talk or Teacher Talk: A 3-minute informative or ACE-aligned address." },
      { text: "Coordinator announces changes in schedule and sets a Positive Intention for the Day." },
      { text: "Students quietly move to their classrooms with teachers by 8:17 am." },
      { text: "Long Assembly adds: Class Assembly performance, Rewards and Recognition segment." }
    ],
    dos: [
      { text: "Have the sound system ready before students arrive" },
      { text: "Keep assembly brisk : aim to close by 8:17 am" },
      { text: "Review Gratitude and Talk content with students beforehand" },
      { text: "Set a warm, inclusive tone with the Positive Intention" },
      { text: "Assign grade-wise schedule for Gratitude Reflections at term start" }
    ],
    donts: [
      { text: "Don't allow assembly to run beyond scheduled time" },
      { text: "Don't skip the prayer or positive intention : they set the tone" },
      { text: "Don't let students leave lines or become disruptive" },
      { text: "Don't give unreviewed content for student/teacher talks" },
      { text: "Don't forget to check mic and sound system in advance" }
    ],
    glossary: [
      { term: "Short Assembly", mean: "The daily 10-minute assembly format conducted on regular school mornings." },
      { term: "Long Assembly", mean: "An extended assembly held weekly or on special occasions, featuring a class presentation." },
      { term: "ACE", mean: "Aware, Compassionate, Engaged : the three pillars guiding student character and wellbeing." },
      { term: "Positive Intention", mean: "A motivating thought or focus shared by the Coordinator to guide students through the day." },
      { term: "Gratitude Reflection", mean: "A brief moment where students share what they are thankful for, practising mindfulness and positivity." }
    ],
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await settingsService.getSetting("page_culture_school_assembly");
        if (result && result.value) {
          setData(prev => ({ ...prev, ...result.value }));
        }
      } catch (error) {
        console.error("Failed to fetch school assembly content:", error);
      }
    };
    fetchData();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-24 rounded-b-[2.5rem] smooth-scroll">
      <PageEditorControls
        settingKey="page_culture_school_assembly"
        initialData={data}
        onSave={setData}
        title="School Assembly Content"
